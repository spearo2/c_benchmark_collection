# 1 "/doner/libarchive/libarchive-3.2.0/libarchive/archive_ppmd7.c"
...
# 6 "/doner/libarchive/libarchive-3.2.0/libarchive/archive_ppmd7.c" 2
...
# 8 "/doner/libarchive/libarchive-3.2.0/libarchive/archive_ppmd7.c" 2
...
# 10 "/doner/libarchive/libarchive-3.2.0/libarchive/archive_ppmd7.c" 2
# 49 "/doner/libarchive/libarchive-3.2.0/libarchive/archive_ppmd7.c"
...
static void *AllocUnitsRare(CPpmd7 *p, unsigned indx)
{
  ...
  do
  {
    if (++i == (4 + 4 + 4 + ((128 + 3 - 1 * 4 - 2 * 4 - 3 * 4) / 4)))
    {
      ...
      p->GlueCount--;
      return ((UInt32)(p->UnitsStart - p->Text) > numBytes) ? (p->UnitsStart -= numBytes) : (
# 261 "/doner/libarchive/libarchive-3.2.0/libarchive/archive_ppmd7.c" 3 4
                                                                                            ((void *)0)
# 261 "/doner/libarchive/libarchive-3.2.0/libarchive/archive_ppmd7.c"
                                                                                                );
    }
  }
...
static CTX_PTR CreateSuccessors(CPpmd7 *p, Bool skip)
{
  ...
  while (numPs != 0)
  {
    ...
    else
    {
      ...
      if (!c1)
        return 
# 430 "/doner/libarchive/libarchive-3.2.0/libarchive/archive_ppmd7.c" 3 4
              ((void *)0)
# 430 "/doner/libarchive/libarchive-3.2.0/libarchive/archive_ppmd7.c"
                  ;
    }
    c1->NumStats = 1;
  ...
  *p->Text++ = p->FoundState->Symbol;
  ...
  if (fSuccessor)
  {
    if (fSuccessor <= successor)
    {
      ...
      if (cs == 
# 510 "/doner/libarchive/libarchive-3.2.0/libarchive/archive_ppmd7.c" 3 4
               ((void *)0)
# 510 "/doner/libarchive/libarchive-3.2.0/libarchive/archive_ppmd7.c"
                   )
      {
        RestartModel(p);
        return;
...
{
  CPpmd7z_RangeDec *p = (CPpmd7z_RangeDec *)pp;
  UInt32 bit, value = p->p.GetThreshold(p, (1 << (7 + 7)));
  if(value < size0)
  {
    bit = 0;
  ...
  {
    bit = 1;
    p->p.Decode(p, size0, (1 << (7 + 7)) - size0);
  }
  return bit;
}
...
static int Ppmd7_DecodeSymbol(CPpmd7 *p, IPpmd7_RangeDec *rc)
{
  ...
  if (p->MinContext->NumStats != 1)
  {
    CPpmd_State *s = ((CPpmd_State *)((void *)(((p))->Base + (((p->MinContext)->Stats)))));
    unsigned i;
    UInt32 count, hiCnt;
    ...
    { unsigned j; for (j = 0; j < 256 / sizeof(charMask[0]); j += 8) { charMask[j+7] = charMask[j+6] = charMask[j+5] = charMask[j+4] = charMask[j+3] = charMask[j+2] = charMask[j+1] = charMask[j+0] = ~(size_t)0; }};
    ((signed char *)charMask)[s->Symbol] = 0;
    i = p->MinContext->NumStats - 1;
    do { ((signed char *)charMask)[(--s)->Symbol] = 0; } while (--i);
  }
  else
  {
    UInt16 *prob = &p->BinSumm[((CPpmd_State *)&(p->MinContext)->SummFreq)->Freq - 1][p->PrevSuccess + p->NS2BSIndx[((CPpmd7_Context *)((void *)(((p))->Base + ((p->MinContext->Suffix)))))->NumStats - 1] + (p->HiBitsFlag = p->HB2Flag[p->FoundState->Symbol]) + 2 * p->HB2Flag[((CPpmd_State *)&(p->MinContext)->SummFreq)->Symbol] + ((p->RunLength >> 26) & 0x20)];
    if (rc->DecodeBit(rc, *prob) == 0)
    {
      Byte symbol;
      *prob = (UInt16)((*prob) + (1 << 7) - ((((*prob)) + (1 << ((7) - (2)))) >> (7)));
      symbol = (p->FoundState = ((CPpmd_State *)&(p->MinContext)->SummFreq))->Symbol;
      Ppmd7_UpdateBin(p);
      return symbol;
    }
    *prob = (UInt16)((*prob) - ((((*prob)) + (1 << ((7) - (2)))) >> (7)));
    p->InitEsc = PPMD7_kExpEscape[*prob >> 10];
    { unsigned j; for (j = 0; j < 256 / sizeof(charMask[0]); j += 8) { charMask[j+7] = charMask[j+6] = charMask[j+5] = charMask[j+4] = charMask[j+3] = charMask[j+2] = charMask[j+1] = charMask[j+0] = ~(size_t)0; }};
    ((signed char *)charMask)[((CPpmd_State *)&(p->MinContext)->SummFreq)->Symbol] = 0;
    p->PrevSuccess = 0;
  }
  for (;;)
  {
    ...
    do
    {
      int k = (int)(((signed char *)charMask)[s->Symbol]);
      hiCnt += (s->Freq & k);
      ps[i] = s++;
      i -= k;
    }
    ...
    if (count < hiCnt)
    {
      ...
      s = *pps;
      rc->Decode(rc, hiCnt - s->Freq, s->Freq);
      if ((see)->Shift < 7 && --(see)->Count == 0) { (see)->Summ <<= 1; (see)->Count = (Byte)(3 << (see)->Shift++); };
      p->FoundState = s;
      symbol = s->Symbol;
      Ppmd7_Update2(p);
      return symbol;
    ...
    do { ((signed char *)charMask)[ps[--i]->Symbol] = 0; } while (i != 0);
  }
}
...
static void Ppmd7_EncodeSymbol(CPpmd7 *p, CPpmd7z_RangeEnc *rc, int symbol)
{
  ...
  {
    UInt16 *prob = &p->BinSumm[((CPpmd_State *)&(p->MinContext)->SummFreq)->Freq - 1][p->PrevSuccess + p->NS2BSIndx[((CPpmd7_Context *)((void *)(((p))->Base + ((p->MinContext->Suffix)))))->NumStats - 1] + (p->HiBitsFlag = p->HB2Flag[p->FoundState->Symbol]) + 2 * p->HB2Flag[((CPpmd_State *)&(p->MinContext)->SummFreq)->Symbol] + ((p->RunLength >> 26) & 0x20)];
    CPpmd_State *s = ((CPpmd_State *)&(p->MinContext)->SummFreq);
    if (s->Symbol == symbol)
    {
  ...
  for (;;)
  {
    ...
    do
    {
      ...
      if (cur == symbol)
      {
        ...
        do
        {
          sum += (s->Freq & (int)(((signed char *)charMask)[s->Symbol]));
          s++;
        }
        while (--i);
      ...
      sum += (s->Freq & (int)(((signed char *)charMask)[cur]));
      ((signed char *)charMask)[cur] = 0;
      s++;
    }
    while (--i);
