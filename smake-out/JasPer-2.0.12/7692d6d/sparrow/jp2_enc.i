# 1 "/doner/jasper/jasper-7692d6d/src/libjasper/jp2/jp2_enc.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 345 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/doner/jasper/jasper-7692d6d/src/libjasper/jp2/jp2_enc.c" 2
# 74 "/doner/jasper/jasper-7692d6d/src/libjasper/jp2/jp2_enc.c"
...
extern void __assert (const char *__assertion, const char *__file, int __line)
     __attribute__ ((__nothrow__ )) __attribute__ ((__noreturn__));
# 75 "/doner/jasper/jasper-7692d6d/src/libjasper/jp2/jp2_enc.c" 2
...
# 76 "/doner/jasper/jasper-7692d6d/src/libjasper/jp2/jp2_enc.c" 2
...
# 77 "/doner/jasper/jasper-7692d6d/src/libjasper/jp2/jp2_enc.c" 2
...
# 81 "/doner/jasper/jasper-7692d6d/src/libjasper/jp2/jp2_enc.c" 2
...
int jp2_encode(jas_image_t *image, jas_stream_t *out, const char *optstr)
{
 ...
 if (!(box = jp2_box_create(0x6a502020))) {
  jas_eprintf("cannot create JP box\n");
  goto error;
 }
 box->data.jp.magic = 0x0d0a870a;
 ...
 if (!(box = jp2_box_create(0x66747970))) {
  jas_eprintf("cannot create FTYP box\n");
  goto error;
 }
 ftyp = &box->data.ftyp;
 ftyp->majver = 0x6a703220;
 ftyp->minver = 0;
 ftyp->numcompatcodes = 1;
 ftyp->compatcodes[0] = 0x6a703220;
 if (jp2_box_put(box, out)) {
 ...
 jp2_box_destroy(box);
 box = 0;
# 168 "/doner/jasper/jasper-7692d6d/src/libjasper/jp2/jp2_enc.c"
 ...
 if (!(box = jp2_box_create(0x69686472))) {
  jas_eprintf("cannot create IHDR box\n");
  goto error;
 }
 ihdr = &box->data.ihdr;
 ihdr->width = ((image)->brx_ - (image)->tlx_);
 ihdr->height = ((image)->bry_ - (image)->tly_);
 ihdr->numcmpts = ((image)->numcmpts_);
 ihdr->bpc = allcmptssame ? ((((((image)->cmpts_[0]->prec_)) - 1) & 0x7f) | (((((image)->cmpts_[0]->sgnd_)) & 1) << 7)) : 255;
 ...
 if (!allcmptssame) {
  if (!(box = jp2_box_create(0x62706363))) {
   jas_eprintf("cannot create BPCC box\n");
   goto error;
  }
  bpcc = &box->data.bpcc;
  bpcc->numcmpts = ((image)->numcmpts_);
  ...
  for (cmptno = 0; cmptno < bpcc->numcmpts; ++cmptno) {
   bpcc->bpcs[cmptno] = ((((((image)->cmpts_[cmptno]->prec_)) - 1) & 0x7f) | (((((image)->cmpts_[cmptno]->sgnd_)) & 1) << 7));
 ...
 if (!(box = jp2_box_create(0x636f6c72))) {
 ...
 case (((3) << 8) | (1)):
  colr->method = 1;
  colr->csid = clrspctojp2(((image)->clrspc_));
  colr->pri = 0;
  colr->approx = 0;
  break;
 default:
 ...
 case 4:
  if (((image)->cmpts_[(0)]->type_) ==
    ((0) & 0x7fff) &&
    ((image)->cmpts_[(1)]->type_) ==
    ((1) & 0x7fff) &&
    ((image)->cmpts_[(2)]->type_) ==
    ((2) & 0x7fff))
   needcdef = 0;
  break;
 case 5:
 ...
 if (needcdef) {
  if (!(box = jp2_box_create(0x63646566))) {
   jas_eprintf("cannot create CDEF box\n");
   goto error;
  }
  cdef = &box->data.cdef;
  cdef->numchans = ((image)->numcmpts_);
  cdef->ents = jas_alloc2(cdef->numchans, sizeof(jp2_cdefchan_t));
  for (i = 0; i < ((image)->numcmpts_); ++i) {
   cdefchanent = &cdef->ents[i];
   cdefchanent->channo = i;
   typeasoc = jp2_gettypeasoc(((image)->clrspc_), ((image)->cmpts_[(i)]->type_));
 ...
 if (!(box = jp2_box_create(0x6a703268))) {
 ...
 if (!(box = jp2_box_create(0x6a703263))) {
 ...
 overhead = (((const jas_stream_t *)(out))->rwcnt_);
 sprintf(buf, "%s\n_jp2overhead=%lu\n", (optstr ? optstr : ""),
