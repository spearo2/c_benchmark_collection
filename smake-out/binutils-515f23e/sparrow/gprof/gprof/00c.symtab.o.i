# 1 "/doner/binutils/binutils-515f23e/gprof/symtab.c"
...
# 1 "<command-line>" 2
# 1 "/doner/binutils/binutils-515f23e/gprof/symtab.c"
# 22 "/doner/binutils/binutils-515f23e/gprof/symtab.c"
...
# 23 "/doner/binutils/binutils-515f23e/gprof/symtab.c" 2
...
# 24 "/doner/binutils/binutils-515f23e/gprof/symtab.c" 2
...
extern FILE *annotate_source
  (Source_File *sf, unsigned int max_width,
    ...
    void *arg);
# 25 "/doner/binutils/binutils-515f23e/gprof/symtab.c" 2
...
# 26 "/doner/binutils/binutils-515f23e/gprof/symtab.c" 2
...
# 27 "/doner/binutils/binutils-515f23e/gprof/symtab.c" 2
...
# 28 "/doner/binutils/binutils-515f23e/gprof/symtab.c" 2
...
# 60 "/doner/binutils/binutils-515f23e/gprof/symtab.c"
static int
...
symtab_finalize (Sym_Table *tab)
{
   ...
   if ((!src->is_static && dst[-1].is_static)
    ...
    && ((src->is_func && !dst[-1].is_func)
     ...
     && ((src->name[0] != '_' && dst[-1].name[0] == '_')
      ...
      && dst[-1].name[1] == '_'))))))
     {
       if (debug_level & ((1<<7) | (1<<12))) {printf ("[symtab_finalize] favor %s@%c%c over %s@%c%c", src->name, src->is_static ? 't' : 'T', src->is_func ? 'F' : 'f', dst[-1].name, dst[-1].is_static ? 't' : 'T', dst[-1].is_func ? 'F' : 'f'); printf (" (addr=%lx)\n", (unsigned long) src->addr);}
     ...
     {
       if (debug_level & ((1<<7) | (1<<12))) {printf ("[symtab_finalize] favor %s@%c%c over %s@%c%c", dst[-1].name, dst[-1].is_static ? 't' : 'T', dst[-1].is_func ? 'F' : 'f', src->name, src->is_static ? 't' : 'T', src->is_func ? 'F' : 'f'); printf (" (addr=%lx)\n", (unsigned long) src->addr);}
      ...
      else
 {
  ...
  if (debug_level & ((1<<7) | (1<<12))) {printf ("[symtab_finalize]: removed %d duplicate entries\n", tab->len - (int) (dst - tab->base));}
  ...
  if (debug_level & ((1<<7) | (1<<12))) {unsigned int j; for (j = 0; j < tab->len; ++j) { printf ("[symtab_finalize] 0x%lx-0x%lx\t%s\n", (unsigned long) tab->base[j].addr, (unsigned long) tab->base[j].end_addr, tab->base[j].name); };}
# 171 "/doner/binutils/binutils-515f23e/gprof/symtab.c"
   ;
}
...
dbg_sym_lookup (Sym_Table *sym_tab, bfd_vma address)
{
  ...
  fprintf (
# 183 "/doner/binutils/binutils-515f23e/gprof/symtab.c" 3 4
          stderr
# 183 "/doner/binutils/binutils-515f23e/gprof/symtab.c"
  ...
  for (low = 0, high = sym_tab->len - 1; low != high;)
    {
      ...
      fprintf (
# 191 "/doner/binutils/binutils-515f23e/gprof/symtab.c" 3 4
              stderr
# 191 "/doner/binutils/binutils-515f23e/gprof/symtab.c"
                    , "[dbg_sym_lookup] low=0x%lx, mid=0x%lx, high=0x%lx\n",
        low, mid, high);
      fprintf (
# 193 "/doner/binutils/binutils-515f23e/gprof/symtab.c" 3 4
              stderr
# 193 "/doner/binutils/binutils-515f23e/gprof/symtab.c"
 ...
 low = mid + 1;
  ...
  fprintf (
# 206 "/doner/binutils/binutils-515f23e/gprof/symtab.c" 3 4
          stderr
# 206 "/doner/binutils/binutils-515f23e/gprof/symtab.c"
...
sym_lookup (Sym_Table *sym_tab, bfd_vma address)
{
      ...
      if (sym[mid].addr <= address && sym[mid + 1].addr > address)
 {
   if (address > sym[mid].end_addr)
     ...
     {
       if (debug_level & ((1<<9))) {printf ("[sym_lookup] %d probes (symtab->len=%u)\n", probes, sym_tab->len - 1);}
 ...
 {
   if (debug_level & ((1<<9))) {printf ("[sym_lookup] %d (%u) probes, fall off\n", probes, sym_tab->len - 1);}
                                  ;
   return &sym[mid + 1];
 }
    }
