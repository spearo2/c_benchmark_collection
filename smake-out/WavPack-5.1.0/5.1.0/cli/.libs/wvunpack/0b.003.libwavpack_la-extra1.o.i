# 1 "/doner/wavpack/wavpack-5.1.0/src/extra1.c"
...
# 1 "<command-line>" 2
# 1 "/doner/wavpack/wavpack-5.1.0/src/extra1.c"
# 13 "/doner/wavpack/wavpack-5.1.0/src/extra1.c"
...
# 14 "/doner/wavpack/wavpack-5.1.0/src/extra1.c" 2
...
# 15 "/doner/wavpack/wavpack-5.1.0/src/extra1.c" 2
...
# 16 "/doner/wavpack/wavpack-5.1.0/src/extra1.c" 2
...
# 17 "/doner/wavpack/wavpack-5.1.0/src/extra1.c" 2
...
# 19 "/doner/wavpack/wavpack-5.1.0/src/extra1.c" 2
# 46 "/doner/wavpack/wavpack-5.1.0/src/extra1.c"
...
static void decorr_mono_pass (int32_t *in_samples, int32_t *out_samples, uint32_t num_samples, struct decorr_pass *dpp, int dir)
{
    ...
    if (num_samples > 16 && dir > 0) {
        int32_t pre_samples = (dpp->term > 8) ? 2 : dpp->term;
    ...
    if (dpp->term > 8) {
        while (num_samples--) {
            ...
            left -= (sam_A != (int16_t) sam_A ? (((((sam_A & 0xffff) * dpp->weight_A) >> 9) + (((sam_A & ~0xffff) >> 9) * dpp->weight_A) + 1) >> 1) : ((dpp->weight_A * sam_A + 512) >> 10));
    ...
    if (cont_samples)
        pack_decorr_mono_pass_cont_x64 (out_samples, in_samples, dpp, cont_samples);
...
static void recurse_mono (WavpackContext *wpc, WavpackExtraInfo *info, int depth, int delta, uint32_t input_bits)
{
    ...
    for (term = 1; term <= 18; ++term) {
        ...
        bits = log2buffer_x64 (outsamples, wps->wphdr.block_samples, info->log_limit);
...
static void delta_mono (WavpackContext *wpc, WavpackExtraInfo *info)
{
    ...
    for (d = delta - 1; d >= 0; --d) {
        ...
        bits = log2buffer_x64 (info->sampleptrs [i], wps->wphdr.block_samples, info->log_limit);
...
static void analyze_mono (WavpackContext *wpc, int32_t *samples, int do_samples)
{
    ...
    info.best_bits = log2buffer_x64 (info.sampleptrs [info.nterms], wps->wphdr.block_samples, 0) * 1;
    ...
    if (wpc->config.extra_flags & 0x1c0)
        recurse_mono (wpc, &info, 0, (int) floor (wps->delta_decay + 0.5),
            log2buffer_x64 (info.sampleptrs [0], wps->wphdr.block_samples, 0));
...
static void mono_add_noise (WavpackStream *wps, int32_t *lptr, int32_t *rptr)
{
    ...
    if (wps->wphdr.flags & 0x40) {
        while (cnt--) {
            ...
            temp = -(error != (int16_t) error ? (((((error & 0xffff) * shaping_weight) >> 9) + (((error & ~0xffff) >> 9) * shaping_weight) + 1) >> 1) : ((shaping_weight * error + 512) >> 10));
...
# 522 "/doner/wavpack/wavpack-5.1.0/src/extra1.c" 3 4
                                                           ((void *)0)
# 522 "/doner/wavpack/wavpack-5.1.0/src/extra1.c"
                                                               ;
    ...
    for (pi = 0; pi < wps->num_passes;) {
        ...
        size = log2buffer_x64 (temp_buffer [j&1], num_samples, log_limit);
    ...
    free (temp_buffer [0]);
    free (best_buffer);
# 703 "/doner/wavpack/wavpack-5.1.0/src/extra1.c"
}
