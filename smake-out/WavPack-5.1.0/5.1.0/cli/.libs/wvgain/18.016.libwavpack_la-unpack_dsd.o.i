# 1 "/doner/wavpack/wavpack-5.1.0/src/unpack_dsd.c"
...
# 1 "<command-line>" 2
# 1 "/doner/wavpack/wavpack-5.1.0/src/unpack_dsd.c"
# 15 "/doner/wavpack/wavpack-5.1.0/src/unpack_dsd.c"
...
# 16 "/doner/wavpack/wavpack-5.1.0/src/unpack_dsd.c" 2
...
# 17 "/doner/wavpack/wavpack-5.1.0/src/unpack_dsd.c" 2
...
# 18 "/doner/wavpack/wavpack-5.1.0/src/unpack_dsd.c" 2
...
# 20 "/doner/wavpack/wavpack-5.1.0/src/unpack_dsd.c" 2
...
int init_dsd_block (WavpackContext *wpc, WavpackMetadata *wpmd)
{
    ...
    if (!wps->dsd.mode) {
        if (wps->dsd.endptr - wps->dsd.byteptr != wps->wphdr.block_samples * (wps->wphdr.flags & (4 | 0x40000000) ? 1 : 2)) {
            return 0;
        }
...
int32_t unpack_dsd_samples (WavpackContext *wpc, int32_t *buffer, uint32_t sample_count)
{
    ...
    if (!wps->mute_error) {
        if (!wps->dsd.mode) {
            int total_samples = sample_count * ((flags & (4 | 0x40000000)) ? 1 : 2);
            int32_t *bptr = buffer;
...
# 129 "/doner/wavpack/wavpack-5.1.0/src/unpack_dsd.c"
static int init_dsd_block_fast (WavpackStream *wps, WavpackMetadata *wpmd)
{
    ...
    if (wps->dsd.byteptr == wps->dsd.endptr || history_bits > 5)
        return 0;
...
static int decode_fast (WavpackStream *wps, int32_t *output, int sample_count)
{
    ...
    while (total_samples--) {
        ...
        while ((!(((wps->dsd.high) ^ (wps->dsd.low)) & 0xff000000)) && wps->dsd.byteptr < wps->dsd.endptr) {
            wps->dsd.value = (wps->dsd.value << 8) | *wps->dsd.byteptr++;
...
# 285 "/doner/wavpack/wavpack-5.1.0/src/unpack_dsd.c"
static void init_ptable (int *table, int rate_i, int rate_s)
{
    int value = 0x808000, rate = rate_i << 8, c, i;

    for (c = (rate + 128) >> 8; c--;)
        value += (0x00010000 - value) >> 8;

    for (i = 0; i < (1<<8)/2; ++i) {
        table [i] = value;
        table [(1<<8)-1-i] = 0x100ffff - value;
...
static int init_dsd_block_high (WavpackStream *wps, WavpackMetadata *wpmd)
{
    ...
    if (wps->dsd.endptr - wps->dsd.byteptr < ((flags & (4 | 0x40000000)) ? 13 : 20))
        return 0;
    ...
    wps->dsd.ptable = malloc ((1<<8) * sizeof (*wps->dsd.ptable));
    init_ptable (wps->dsd.ptable, rate_i, rate_s);

    for (channel = 0; channel < ((flags & (4 | 0x40000000)) ? 1 : 2); ++channel) {
        DSDfilters *sp = wps->dsd.filters + channel;

        sp->filter1 = *wps->dsd.byteptr++ << (20 - 8);
        sp->filter2 = *wps->dsd.byteptr++ << (20 - 8);
        sp->filter3 = *wps->dsd.byteptr++ << (20 - 8);
        sp->filter4 = *wps->dsd.byteptr++ << (20 - 8);
        sp->filter5 = *wps->dsd.byteptr++ << (20 - 8);
        sp->filter6 = 0;
...
static int decode_high (WavpackStream *wps, int32_t *output, int sample_count)
{
    int total_samples = sample_count, stereo = (wps->wphdr.flags & (4 | 0x40000000)) ? 0 : 1;
    ...
    while (total_samples--) {
        ...
        while (bitcount--) {
            int32_t *pp = wps->dsd.ptable + ((sp [0].value >> (20 - 12)) & ((1<<8)-1));
            ...
            sp [0].filter1 += ((sp [0].filter0 & (1 << 20)) - sp [0].filter1) >> 6;
            sp [0].filter2 += ((sp [0].filter0 & (1 << 20)) - sp [0].filter2) >> 4;
            ...
            pp = wps->dsd.ptable + ((sp [1].value >> (20 - 12)) & ((1<<8)-1));
            ...
            sp [1].filter1 += ((sp [1].filter0 & (1 << 20)) - sp [1].filter1) >> 6;
            sp [1].filter2 += ((sp [1].filter0 & (1 << 20)) - sp [1].filter2) >> 4;
...
# 470 "/doner/wavpack/wavpack-5.1.0/src/unpack_dsd.c"
...
typedef struct {
    unsigned char delay [((56 +7)/8)];
} DecimationChannel;

typedef struct {
    int32_t conv_tables [((56 +7)/8)] [256];
...
void *decimate_dsd_init (int num_channels)
{
    ...
    if (!context->chans) {
        free (context);
        return 
# 511 "/doner/wavpack/wavpack-5.1.0/src/unpack_dsd.c" 3 4
              ((void *)0)
# 511 "/doner/wavpack/wavpack-5.1.0/src/unpack_dsd.c"
                  ;
    }
...
void decimate_dsd_run (void *decimate_context, int32_t *samples, int num_samples)
{
    ...
    while (num_samples) {
        ...
        int sum = 0;
# 578 "/doner/wavpack/wavpack-5.1.0/src/unpack_dsd.c"
...
# 594 "/doner/wavpack/wavpack-5.1.0/src/unpack_dsd.c"
        *samples++ = sum >> 4;
