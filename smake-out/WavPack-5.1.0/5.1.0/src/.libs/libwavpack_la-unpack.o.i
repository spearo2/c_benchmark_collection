# 1 "/doner/wavpack/wavpack-5.1.0/src/unpack.c"
...
# 1 "<command-line>" 2
# 1 "/doner/wavpack/wavpack-5.1.0/src/unpack.c"
# 16 "/doner/wavpack/wavpack-5.1.0/src/unpack.c"
...
# 17 "/doner/wavpack/wavpack-5.1.0/src/unpack.c" 2
...
# 18 "/doner/wavpack/wavpack-5.1.0/src/unpack.c" 2
...
# 20 "/doner/wavpack/wavpack-5.1.0/src/unpack.c" 2
# 40 "/doner/wavpack/wavpack-5.1.0/src/unpack.c"
...
# 70 "/doner/wavpack/wavpack-5.1.0/src/unpack.c"
...
int32_t unpack_samples (WavpackContext *wpc, int32_t *buffer, uint32_t sample_count)
{
    ...
    int32_t mute_limit = (1L << ((flags & (0x1fL << 18)) >> 18)) + 2;
    ...
    if (wps->sample_index + sample_count > ( (int64_t) (wps->wphdr).block_index + ((int64_t) (wps->wphdr).block_index_u8 << 32) ) + wps->wphdr.block_samples &&
        ( (int64_t) (wps->wphdr).block_index + ((int64_t) (wps->wphdr).block_index_u8 << 32) ) + wps->wphdr.block_samples - wps->sample_index < sample_count)
            sample_count = (uint32_t) (( (int64_t) (wps->wphdr).block_index + ((int64_t) (wps->wphdr).block_index_u8 << 32) ) + wps->wphdr.block_samples - wps->sample_index);

    if (( (int64_t) (wps->wphdr).block_index + ((int64_t) (wps->wphdr).block_index_u8 << 32) ) > wps->sample_index || wps->wphdr.block_samples < sample_count)
        wps->mute_error = 1;

    if (wps->mute_error) {
        if (wpc->reduced_channels == 1 || wpc->config.num_channels == 1 || (flags & 4))
    ...
    if ((flags & 8) && !wps->block2buff)
        mute_limit = (mute_limit * 2) + 128;



    if (!wps->block2buff && (flags & (4 | 0x40000000))) {
        ...
        if (flags & 8) {
            ...
            for (bptr = buffer; bptr < eptr;)
                if ((*bptr++ = get_word (wps, 0, 
# 114 "/doner/wavpack/wavpack-5.1.0/src/unpack.c" 3 4
                                                ((void *)0)
# 114 "/doner/wavpack/wavpack-5.1.0/src/unpack.c"
                                                    )) == ((int32_t)(1L << 31))) {
        ...
        if (sample_count < 16)
            ...
            for (tcount = wps->num_terms, dpp = wps->decorr_passes; tcount--; dpp++) {
                int pre_samples = (dpp->term > 8) ? 2 : dpp->term;

                decorr_mono_pass (dpp, buffer, pre_samples);

                unpack_decorr_mono_pass_cont_x64 (dpp, buffer + pre_samples, sample_count - pre_samples,
                    ((flags & (0x1fL << 18)) >> 18) > 15);
            }
# 143 "/doner/wavpack/wavpack-5.1.0/src/unpack.c"
    ...
    else if (!wps->block2buff && !(flags & (4 | 0x40000000))) {
        ...
        if (flags & 8) {
            ...
            for (bptr = buffer; bptr < eptr; bptr += 2)
                if ((bptr [0] = get_word (wps, 0, 
# 167 "/doner/wavpack/wavpack-5.1.0/src/unpack.c" 3 4
                                                 ((void *)0)
# 167 "/doner/wavpack/wavpack-5.1.0/src/unpack.c"
                                                     )) == ((int32_t)(1L << 31)) ||
                    (bptr [1] = get_word (wps, 1, 
# 168 "/doner/wavpack/wavpack-5.1.0/src/unpack.c" 3 4
                                                 ((void *)0)
# 168 "/doner/wavpack/wavpack-5.1.0/src/unpack.c"
                                                     )) == ((int32_t)(1L << 31))) {
        ...
        if (sample_count < 16 || !1) {
            ...
            for (tcount = wps->num_terms, dpp = wps->decorr_passes; tcount--; dpp++) {
                int pre_samples = (dpp->term < 0 || dpp->term > 8) ? 2 : dpp->term;

                decorr_stereo_pass (dpp, buffer, pre_samples);

                unpack_decorr_stereo_pass_cont_x64 (dpp, buffer + pre_samples * 2, sample_count - pre_samples,
                    ((flags & (0x1fL << 18)) >> 18) >= 16);
            }
        ...
        for (bptr = buffer, i = 0; i < sample_count; ++i) {
            ...
            if ((read_word = get_word (wps, 0, correction)) == ((int32_t)(1L << 31)))
                break;
            ...
            for (tcount = wps->num_terms, dpp = wps->decorr_passes; tcount--; dpp++) {
                ...
                temp = (sam != (int16_t) sam ? (((((sam & 0xffff) * dpp->weight_A) >> 9) + (((sam & ~0xffff) >> 9) * dpp->weight_A) + 1) >> 1) : ((dpp->weight_A * sam + 512) >> 10)) + read_word;
                if (sam && read_word) { int32_t s = (int32_t) (sam ^ read_word) >> 31; dpp->weight_A = (dpp->delta ^ s) + (dpp->weight_A - s); };
            ...
            if (flags & 0x40) {
                ...
                int32_t temp = -(wps->dc.error [0] != (int16_t) wps->dc.error [0] ? (((((wps->dc.error [0] & 0xffff) * shaping_weight) >> 9) + (((wps->dc.error [0] & ~0xffff) >> 9) * shaping_weight) + 1) >> 1) : ((shaping_weight * wps->dc.error [0] + 512) >> 10));
        ...
        for (bptr = buffer, i = 0; i < sample_count; ++i) {
            ...
            if ((left = get_word (wps, 0, correction)) == ((int32_t)(1L << 31)) ||
                (right = get_word (wps, 1, correction + 1)) == ((int32_t)(1L << 31)))
                    break;
            ...
            if (flags & 0x20) {
                ...
                for (tcount = wps->num_terms, dpp = wps->decorr_passes; tcount--; dpp++) {
                    ...
                    if (dpp->term > 0) {
                        ...
                        left_c += (sam_A != (int16_t) sam_A ? (((((sam_A & 0xffff) * dpp->weight_A) >> 9) + (((sam_A & ~0xffff) >> 9) * dpp->weight_A) + 1) >> 1) : ((dpp->weight_A * sam_A + 512) >> 10));
                        right_c += (sam_B != (int16_t) sam_B ? (((((sam_B & 0xffff) * dpp->weight_B) >> 9) + (((sam_B & ~0xffff) >> 9) * dpp->weight_B) + 1) >> 1) : ((dpp->weight_B * sam_B + 512) >> 10));
                    }
                    else if (dpp->term == -1) {
                        left_c += (dpp->samples_A [0] != (int16_t) dpp->samples_A [0] ? (((((dpp->samples_A [0] & 0xffff) * dpp->weight_A) >> 9) + (((dpp->samples_A [0] & ~0xffff) >> 9) * dpp->weight_A) + 1) >> 1) : ((dpp->weight_A * dpp->samples_A [0] + 512) >> 10));
                        right_c += (left_c != (int16_t) left_c ? (((((left_c & 0xffff) * dpp->weight_B) >> 9) + (((left_c & ~0xffff) >> 9) * dpp->weight_B) + 1) >> 1) : ((dpp->weight_B * left_c + 512) >> 10));
                    }
                    else {
                        right_c += (dpp->samples_B [0] != (int16_t) dpp->samples_B [0] ? (((((dpp->samples_B [0] & 0xffff) * dpp->weight_B) >> 9) + (((dpp->samples_B [0] & ~0xffff) >> 9) * dpp->weight_B) + 1) >> 1) : ((dpp->weight_B * dpp->samples_B [0] + 512) >> 10));

                        if (dpp->term == -3)
                            left_c += (dpp->samples_A [0] != (int16_t) dpp->samples_A [0] ? (((((dpp->samples_A [0] & 0xffff) * dpp->weight_A) >> 9) + (((dpp->samples_A [0] & ~0xffff) >> 9) * dpp->weight_A) + 1) >> 1) : ((dpp->weight_A * dpp->samples_A [0] + 512) >> 10));
                        else
                            left_c += (right_c != (int16_t) right_c ? (((((right_c & 0xffff) * dpp->weight_A) >> 9) + (((right_c & ~0xffff) >> 9) * dpp->weight_A) + 1) >> 1) : ((dpp->weight_A * right_c + 512) >> 10));
                    }
                }
            ...
            for (tcount = wps->num_terms, dpp = wps->decorr_passes; tcount--; dpp++) {
                ...
                if (dpp->term > 0) {
                    ...
                    left2 = (sam_A != (int16_t) sam_A ? (((((sam_A & 0xffff) * dpp->weight_A) >> 9) + (((sam_A & ~0xffff) >> 9) * dpp->weight_A) + 1) >> 1) : ((dpp->weight_A * sam_A + 512) >> 10)) + left;
                    right2 = (sam_B != (int16_t) sam_B ? (((((sam_B & 0xffff) * dpp->weight_B) >> 9) + (((sam_B & ~0xffff) >> 9) * dpp->weight_B) + 1) >> 1) : ((dpp->weight_B * sam_B + 512) >> 10)) + right;

                    if (sam_A && left) { int32_t s = (int32_t) (sam_A ^ left) >> 31; dpp->weight_A = (dpp->delta ^ s) + (dpp->weight_A - s); };
                    if (sam_B && right) { int32_t s = (int32_t) (sam_B ^ right) >> 31; dpp->weight_B = (dpp->delta ^ s) + (dpp->weight_B - s); };

                    dpp->samples_A [k] = left = left2;
                    dpp->samples_B [k] = right = right2;
                }
                else if (dpp->term == -1) {
                    left2 = left + (dpp->samples_A [0] != (int16_t) dpp->samples_A [0] ? (((((dpp->samples_A [0] & 0xffff) * dpp->weight_A) >> 9) + (((dpp->samples_A [0] & ~0xffff) >> 9) * dpp->weight_A) + 1) >> 1) : ((dpp->weight_A * dpp->samples_A [0] + 512) >> 10));
                    ...
                    left = left2;
                    right2 = right + (left2 != (int16_t) left2 ? (((((left2 & 0xffff) * dpp->weight_B) >> 9) + (((left2 & ~0xffff) >> 9) * dpp->weight_B) + 1) >> 1) : ((dpp->weight_B * left2 + 512) >> 10));
                    if (left2 && right) { const int32_t s = (left2 ^ right) >> 31; if ((dpp->weight_B = (dpp->weight_B ^ s) + (dpp->delta - s)) > 1024) dpp->weight_B = 1024; dpp->weight_B = (dpp->weight_B ^ s) - s; };
                    dpp->samples_A [0] = right = right2;
                }
                else {
                    right2 = right + (dpp->samples_B [0] != (int16_t) dpp->samples_B [0] ? (((((dpp->samples_B [0] & 0xffff) * dpp->weight_B) >> 9) + (((dpp->samples_B [0] & ~0xffff) >> 9) * dpp->weight_B) + 1) >> 1) : ((dpp->weight_B * dpp->samples_B [0] + 512) >> 10));
                    ...
                    left2 = left + (right2 != (int16_t) right2 ? (((((right2 & 0xffff) * dpp->weight_A) >> 9) + (((right2 & ~0xffff) >> 9) * dpp->weight_A) + 1) >> 1) : ((dpp->weight_A * right2 + 512) >> 10));
                    if (right2 && left) { const int32_t s = (right2 ^ left) >> 31; if ((dpp->weight_A = (dpp->weight_A ^ s) + (dpp->delta - s)) > 1024) dpp->weight_A = 1024; dpp->weight_A = (dpp->weight_A ^ s) - s; };
    ...
    if (i != sample_count) {
        memset (buffer, 0, sample_count * (flags & 4 ? 4 : 8));
        wps->mute_error = 1;
        i = sample_count;

        if (((&wps->wvxbits)->ptr != 
# 457 "/doner/wavpack/wavpack-5.1.0/src/unpack.c" 3 4
           ((void *)0)
# 457 "/doner/wavpack/wavpack-5.1.0/src/unpack.c"
           ))
    ...
    if ((flags & 0x80) && (wpc->open_flags & 0x10))
        WavpackFloatNormalize (buffer, (flags & (4 | 0x40000000)) ? i : i * 2,
    ...
    switch (dpp->term) {
        ...
        case 17:
            for (bptr = buffer; bptr < eptr; bptr++) {
                sam_A = 2 * dpp->samples_A [0] - dpp->samples_A [1];
                dpp->samples_A [1] = dpp->samples_A [0];
                dpp->samples_A [0] = (sam_A != (int16_t) sam_A ? (((((sam_A & 0xffff) * weight_A) >> 9) + (((sam_A & ~0xffff) >> 9) * weight_A) + 1) >> 1) : ((weight_A * sam_A + 512) >> 10)) + bptr [0];
                if (sam_A && bptr [0]) { int32_t s = (int32_t) (sam_A ^ bptr [0]) >> 31; weight_A = (delta ^ s) + (weight_A - s); };
        ...
        default:
            for (m = 0, k = dpp->term & (8 - 1), bptr = buffer; bptr < eptr; bptr++) {
                sam_A = dpp->samples_A [m];
                dpp->samples_A [k] = (sam_A != (int16_t) sam_A ? (((((sam_A & 0xffff) * weight_A) >> 9) + (((sam_A & ~0xffff) >> 9) * weight_A) + 1) >> 1) : ((weight_A * sam_A + 512) >> 10)) + bptr [0];
                if (sam_A && bptr [0]) { int32_t s = (int32_t) (sam_A ^ bptr [0]) >> 31; weight_A = (delta ^ s) + (weight_A - s); };
            ...
            if (m) {
                ...
                for (k = 0; k < 8; k++, m++)
                    dpp->samples_A [k] = temp_samples [m & (8 - 1)];
            }
...
# 568 "/doner/wavpack/wavpack-5.1.0/src/unpack.c"
static void decorr_stereo_pass (struct decorr_pass *dpp, int32_t *buffer, int32_t sample_count)
{
    ...
    switch (dpp->term) {
        case 17:
            for (bptr = buffer; bptr < eptr; bptr += 2) {
                ...
                sam = 2 * dpp->samples_A [0] - dpp->samples_A [1];
                dpp->samples_A [1] = dpp->samples_A [0];
                bptr [0] = dpp->samples_A [0] = (sam != (int16_t) sam ? (((((sam & 0xffff) * dpp->weight_A) >> 9) + (((sam & ~0xffff) >> 9) * dpp->weight_A) + 1) >> 1) : ((dpp->weight_A * sam + 512) >> 10)) + (tmp = bptr [0]);
                if (sam && tmp) { int32_t s = (int32_t) (sam ^ tmp) >> 31; dpp->weight_A = (dpp->delta ^ s) + (dpp->weight_A - s); };

                sam = 2 * dpp->samples_B [0] - dpp->samples_B [1];
                dpp->samples_B [1] = dpp->samples_B [0];
                bptr [1] = dpp->samples_B [0] = (sam != (int16_t) sam ? (((((sam & 0xffff) * dpp->weight_B) >> 9) + (((sam & ~0xffff) >> 9) * dpp->weight_B) + 1) >> 1) : ((dpp->weight_B * sam + 512) >> 10)) + (tmp = bptr [1]);
                if (sam && tmp) { int32_t s = (int32_t) (sam ^ tmp) >> 31; dpp->weight_B = (dpp->delta ^ s) + (dpp->weight_B - s); };
            }
        ...
        default:
            for (m = 0, k = dpp->term & (8 - 1), bptr = buffer; bptr < eptr; bptr += 2) {
                int32_t sam;

                sam = dpp->samples_A [m];
                dpp->samples_A [k] = (sam != (int16_t) sam ? (((((sam & 0xffff) * dpp->weight_A) >> 9) + (((sam & ~0xffff) >> 9) * dpp->weight_A) + 1) >> 1) : ((dpp->weight_A * sam + 512) >> 10)) + bptr [0];
                if (sam && bptr [0]) { int32_t s = (int32_t) (sam ^ bptr [0]) >> 31; dpp->weight_A = (dpp->delta ^ s) + (dpp->weight_A - s); };
                bptr [0] = dpp->samples_A [k];

                sam = dpp->samples_B [m];
                dpp->samples_B [k] = (sam != (int16_t) sam ? (((((sam & 0xffff) * dpp->weight_B) >> 9) + (((sam & ~0xffff) >> 9) * dpp->weight_B) + 1) >> 1) : ((dpp->weight_B * sam + 512) >> 10)) + bptr [1];
                if (sam && bptr [1]) { int32_t s = (int32_t) (sam ^ bptr [1]) >> 31; dpp->weight_B = (dpp->delta ^ s) + (dpp->weight_B - s); };
        ...
        case -1:
            for (bptr = buffer; bptr < eptr; bptr += 2) {
                ...
                sam = bptr [0] + (dpp->samples_A [0] != (int16_t) dpp->samples_A [0] ? (((((dpp->samples_A [0] & 0xffff) * dpp->weight_A) >> 9) + (((dpp->samples_A [0] & ~0xffff) >> 9) * dpp->weight_A) + 1) >> 1) : ((dpp->weight_A * dpp->samples_A [0] + 512) >> 10));
                if (dpp->samples_A [0] && bptr [0]) { const int32_t s = (dpp->samples_A [0] ^ bptr [0]) >> 31; if ((dpp->weight_A = (dpp->weight_A ^ s) + (dpp->delta - s)) > 1024) dpp->weight_A = 1024; dpp->weight_A = (dpp->weight_A ^ s) - s; };
                bptr [0] = sam;
                dpp->samples_A [0] = bptr [1] + (sam != (int16_t) sam ? (((((sam & 0xffff) * dpp->weight_B) >> 9) + (((sam & ~0xffff) >> 9) * dpp->weight_B) + 1) >> 1) : ((dpp->weight_B * sam + 512) >> 10));
                if (sam && bptr [1]) { const int32_t s = (sam ^ bptr [1]) >> 31; if ((dpp->weight_B = (dpp->weight_B ^ s) + (dpp->delta - s)) > 1024) dpp->weight_B = 1024; dpp->weight_B = (dpp->weight_B ^ s) - s; };
        ...
        case -2:
            for (bptr = buffer; bptr < eptr; bptr += 2) {
                ...
                sam = bptr [1] + (dpp->samples_B [0] != (int16_t) dpp->samples_B [0] ? (((((dpp->samples_B [0] & 0xffff) * dpp->weight_B) >> 9) + (((dpp->samples_B [0] & ~0xffff) >> 9) * dpp->weight_B) + 1) >> 1) : ((dpp->weight_B * dpp->samples_B [0] + 512) >> 10));
                if (dpp->samples_B [0] && bptr [1]) { const int32_t s = (dpp->samples_B [0] ^ bptr [1]) >> 31; if ((dpp->weight_B = (dpp->weight_B ^ s) + (dpp->delta - s)) > 1024) dpp->weight_B = 1024; dpp->weight_B = (dpp->weight_B ^ s) - s; };
                bptr [1] = sam;
                dpp->samples_B [0] = bptr [0] + (sam != (int16_t) sam ? (((((sam & 0xffff) * dpp->weight_A) >> 9) + (((sam & ~0xffff) >> 9) * dpp->weight_A) + 1) >> 1) : ((dpp->weight_A * sam + 512) >> 10));
                if (sam && bptr [0]) { const int32_t s = (sam ^ bptr [0]) >> 31; if ((dpp->weight_A = (dpp->weight_A ^ s) + (dpp->delta - s)) > 1024) dpp->weight_A = 1024; dpp->weight_A = (dpp->weight_A ^ s) - s; };
        ...
        case -3:
            for (bptr = buffer; bptr < eptr; bptr += 2) {
                ...
                sam_A = bptr [0] + (dpp->samples_A [0] != (int16_t) dpp->samples_A [0] ? (((((dpp->samples_A [0] & 0xffff) * dpp->weight_A) >> 9) + (((dpp->samples_A [0] & ~0xffff) >> 9) * dpp->weight_A) + 1) >> 1) : ((dpp->weight_A * dpp->samples_A [0] + 512) >> 10));
                if (dpp->samples_A [0] && bptr [0]) { const int32_t s = (dpp->samples_A [0] ^ bptr [0]) >> 31; if ((dpp->weight_A = (dpp->weight_A ^ s) + (dpp->delta - s)) > 1024) dpp->weight_A = 1024; dpp->weight_A = (dpp->weight_A ^ s) - s; };
                sam_B = bptr [1] + (dpp->samples_B [0] != (int16_t) dpp->samples_B [0] ? (((((dpp->samples_B [0] & 0xffff) * dpp->weight_B) >> 9) + (((dpp->samples_B [0] & ~0xffff) >> 9) * dpp->weight_B) + 1) >> 1) : ((dpp->weight_B * dpp->samples_B [0] + 512) >> 10));
                if (dpp->samples_B [0] && bptr [1]) { const int32_t s = (dpp->samples_B [0] ^ bptr [1]) >> 31; if ((dpp->weight_B = (dpp->weight_B ^ s) + (dpp->delta - s)) > 1024) dpp->weight_B = 1024; dpp->weight_B = (dpp->weight_B ^ s) - s; };
...
# 680 "/doner/wavpack/wavpack-5.1.0/src/unpack.c"
static void fixup_samples (WavpackContext *wpc, int32_t *buffer, uint32_t sample_count)
{
    WavpackStream *wps = wpc->streams [wpc->current_stream];
    uint32_t flags = wps->wphdr.flags;
    int lossy_flag = (flags & 8) && !wps->block2buff;
    int shift = (flags & (0x1fL << 13)) >> 13;

    if (flags & 0x80) {
        float_values (wps, buffer, (flags & (4 | 0x40000000)) ? sample_count : sample_count * 2);
        return;
    }

    if (flags & 0x100) {
        uint32_t count = (flags & (4 | 0x40000000)) ? sample_count : sample_count * 2;
        ...
        if (((&wps->wvxbits)->ptr != 
# 699 "/doner/wavpack/wavpack-5.1.0/src/unpack.c" 3 4
           ((void *)0)
# 699 "/doner/wavpack/wavpack-5.1.0/src/unpack.c"
           )) {
            uint32_t crc = wps->crc_x;

            while (count--) {

                    do { while ((sent_bits) > (&wps->wvxbits)->bc) { if (++((&wps->wvxbits)->ptr) == (&wps->wvxbits)->end) (&wps->wvxbits)->wrap (&wps->wvxbits); (&wps->wvxbits)->sr |= (int32_t)*((&wps->wvxbits)->ptr) << (&wps->wvxbits)->bc; (&wps->wvxbits)->bc += sizeof (*((&wps->wvxbits)->ptr)) * 8; } *(&data) = (&wps->wvxbits)->sr; if ((&wps->wvxbits)->bc > 32) { (&wps->wvxbits)->bc -= (sent_bits); (&wps->wvxbits)->sr = *((&wps->wvxbits)->ptr) >> (sizeof (*((&wps->wvxbits)->ptr)) * 8 - (&wps->wvxbits)->bc); } else { (&wps->wvxbits)->bc -= (sent_bits); (&wps->wvxbits)->sr >>= (sent_bits); } } while (0);
            ...
            while (lossy_flag && (flags & 3) == 3 && shift < 8) {
                if (zeros)
                    zeros--;
                else if (ones)
                    ones--;
...
# 803 "/doner/wavpack/wavpack-5.1.0/src/unpack.c"
int check_crc_error (WavpackContext *wpc)
{
    int result = 0, stream;
    ...
    for (stream = 0; stream < wpc->num_streams; stream++) {
        ...
        if (wps->crc != wps->wphdr.crc)
            ++result;
        else if (((&wps->wvxbits)->ptr != 
# 812 "/doner/wavpack/wavpack-5.1.0/src/unpack.c" 3 4
                ((void *)0)
# 812 "/doner/wavpack/wavpack-5.1.0/src/unpack.c"
                ) && wps->crc_x != wps->crc_wvx)
            ++result;
    }
