# 1 "/doner/wavpack/wavpack-5.1.0/src/unpack_seek.c"
...
# 1 "<command-line>" 2
# 1 "/doner/wavpack/wavpack-5.1.0/src/unpack_seek.c"
# 16 "/doner/wavpack/wavpack-5.1.0/src/unpack_seek.c"
...
# 17 "/doner/wavpack/wavpack-5.1.0/src/unpack_seek.c" 2
...
# 18 "/doner/wavpack/wavpack-5.1.0/src/unpack_seek.c" 2
...
# 20 "/doner/wavpack/wavpack-5.1.0/src/unpack_seek.c" 2



static int64_t find_sample (WavpackContext *wpc, void *infile, int64_t header_pos, int64_t sample);
# 33 "/doner/wavpack/wavpack-5.1.0/src/unpack_seek.c"
int WavpackSeekSample (WavpackContext *wpc, uint32_t sample)
{
...
# 40 "/doner/wavpack/wavpack-5.1.0/src/unpack_seek.c" 3 4
                                                                                ((void *)0)
# 40 "/doner/wavpack/wavpack-5.1.0/src/unpack_seek.c"
                                                                                    ;
    uint32_t bcount, samples_to_skip, samples_to_decode = 0;
    int32_t *buffer;

    if (wpc->total_samples == -1 || sample >= wpc->total_samples ||
        !wpc->reader->can_seek (wpc->wv_in) || (wpc->open_flags & 0x20) ||
    ...
    if (!wps->wphdr.block_samples || !(wps->wphdr.flags & 0x800) || sample < ( (int64_t) (wps->wphdr).block_index + ((int64_t) (wps->wphdr).block_index_u8 << 32) ) ||
        sample >= ( (int64_t) (wps->wphdr).block_index + ((int64_t) (wps->wphdr).block_index_u8 << 32) ) + wps->wphdr.block_samples) {
    ...
    if (!wps->blockbuff) {
        ...
        WavpackLittleEndianToNative (&wps->wphdr, "4LS2LLLLL");
    ...
    while (!wpc->reduced_channels && !(wps->wphdr.flags & 0x1000)) {
    ...
    if (sample < wps->sample_index) {
        for (wpc->current_stream = 0; wpc->current_stream < wpc->num_streams; wpc->current_stream++)
            if (!unpack_init (wpc))
                ...
                wpc->streams [wpc->current_stream]->init_done = 1;
    }
    ...
    if (samples_to_skip) {
        ...
        for (wpc->current_stream = 0; wpc->current_stream < wpc->num_streams; wpc->current_stream++)
            ...
            if (wpc->streams [wpc->current_stream]->wphdr.flags & 0x80000000)
...
# 249 "/doner/wavpack/wavpack-5.1.0/src/unpack_seek.c"
static int64_t find_header (WavpackStreamReader64 *reader, void *id, int64_t filepos, WavpackHeader *wphdr)
{
    unsigned char *buffer = malloc (4096), *sp = buffer, *ep = buffer;
    ...
    while (1) {
        ...
        if (sp < ep) {
            ...
            if (sp > ep)
                if (reader->set_pos_rel (id, (int32_t)(sp - ep), 
# 269 "/doner/wavpack/wavpack-5.1.0/src/unpack_seek.c" 3 4
                                                                1
# 269 "/doner/wavpack/wavpack-5.1.0/src/unpack_seek.c"
                                                                        )) {
                    free (buffer);
                    return -1;
                }
        ...
        ep += reader->read_bytes (id, ep, 4096 - bleft);
        ...
        while (sp + 32 <= ep)
            if (*sp++ == 'w' && *sp == 'v' && *++sp == 'p' && *++sp == 'k' &&
                ...
                sp [4] >= (0x402 & 0xff) && sp [4] <= (0x410 & 0xff) && sp [18] < 3 && !sp [19]) {
                    ...
                    if (wphdr->block_samples && (wphdr->flags & 0x800)) {
                        free (buffer);
...
# 311 "/doner/wavpack/wavpack-5.1.0/src/unpack_seek.c"
static int64_t find_sample (WavpackContext *wpc, void *infile, int64_t header_pos, int64_t sample)
{
    ...
    if (header_pos && wps->wphdr.block_samples) {
        if (( (int64_t) (wps->wphdr).block_index + ((int64_t) (wps->wphdr).block_index_u8 << 32) ) > sample) {
            sample_pos2 = ( (int64_t) (wps->wphdr).block_index + ((int64_t) (wps->wphdr).block_index_u8 << 32) );
            file_pos2 = header_pos;
        }
        else if (( (int64_t) (wps->wphdr).block_index + ((int64_t) (wps->wphdr).block_index_u8 << 32) ) + wps->wphdr.block_samples <= sample) {
            sample_pos1 = ( (int64_t) (wps->wphdr).block_index + ((int64_t) (wps->wphdr).block_index_u8 << 32) );
    ...
    while (1) {
        ...
        else if (( (int64_t) (wps->wphdr).block_index + ((int64_t) (wps->wphdr).block_index_u8 << 32) ) > sample) {
            sample_pos2 = ( (int64_t) (wps->wphdr).block_index + ((int64_t) (wps->wphdr).block_index_u8 << 32) );
            file_pos2 = seek_pos;
        }
        else if (( (int64_t) (wps->wphdr).block_index + ((int64_t) (wps->wphdr).block_index_u8 << 32) ) + wps->wphdr.block_samples <= sample) {
