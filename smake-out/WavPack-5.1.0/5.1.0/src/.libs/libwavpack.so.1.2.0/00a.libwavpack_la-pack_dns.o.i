# 1 "/doner/wavpack/wavpack-5.1.0/src/pack_dns.c"
...
# 1 "<command-line>" 2
# 1 "/doner/wavpack/wavpack-5.1.0/src/pack_dns.c"
# 16 "/doner/wavpack/wavpack-5.1.0/src/pack_dns.c"
...
# 17 "/doner/wavpack/wavpack-5.1.0/src/pack_dns.c" 2
...
# 18 "/doner/wavpack/wavpack-5.1.0/src/pack_dns.c" 2
...
# 19 "/doner/wavpack/wavpack-5.1.0/src/pack_dns.c" 2
...
# 21 "/doner/wavpack/wavpack-5.1.0/src/pack_dns.c" 2
...
void dynamic_noise_shaping (WavpackContext *wpc, int32_t *buffer, int shortening_allowed)
{
    ...
    if (!wps->num_terms && sample_count > 8) {
        if (flags & (4 | 0x40000000))
            for (bptr = buffer + sample_count - 3, sc = sample_count - 2; sc--;) {
                ...
                temp = *bptr-- - (sam != (int16_t) sam ? (((((sam & 0xffff) * ap->weight_A) >> 9) + (((sam & ~0xffff) >> 9) * ap->weight_A) + 1) >> 1) : ((ap->weight_A * sam + 512) >> 10));
                if (sam && temp) { int32_t s = (int32_t) (sam ^ temp) >> 31; ap->weight_A = (2 ^ s) + (ap->weight_A - s); };
            }
        else
            for (bptr = buffer + (sample_count - 3) * 2 + 1, sc = sample_count - 2; sc--;) {
                sam = (3 * bptr [2] - bptr [4]) >> 1;
                temp = *bptr-- - (sam != (int16_t) sam ? (((((sam & 0xffff) * ap->weight_B) >> 9) + (((sam & ~0xffff) >> 9) * ap->weight_B) + 1) >> 1) : ((ap->weight_B * sam + 512) >> 10));
                if (sam && temp) { int32_t s = (int32_t) (sam ^ temp) >> 31; ap->weight_B = (2 ^ s) + (ap->weight_B - s); };
                sam = (3 * bptr [2] - bptr [4]) >> 1;
                temp = *bptr-- - (sam != (int16_t) sam ? (((((sam & 0xffff) * ap->weight_A) >> 9) + (((sam & ~0xffff) >> 9) * ap->weight_A) + 1) >> 1) : ((ap->weight_A * sam + 512) >> 10));
                if (sam && temp) { int32_t s = (int32_t) (sam ^ temp) >> 31; ap->weight_A = (2 ^ s) + (ap->weight_A - s); };
            }
    ...
    if (sample_count > wps->dc.shaping_samples) {
        ...
        bptr = buffer + wps->dc.shaping_samples * ((flags & (4 | 0x40000000)) ? 1 : 2);
    ...
    if (wpc->wvc_flag) {
        ...
        if (wpc->wvc_flag) {
            ...
            wps->dc.shaping_array = 
# 142 "/doner/wavpack/wavpack-5.1.0/src/pack_dns.c" 3 4
                                   ((void *)0)
# 142 "/doner/wavpack/wavpack-5.1.0/src/pack_dns.c"
                                       ;
        }
        else
...
# 158 "/doner/wavpack/wavpack-5.1.0/src/pack_dns.c"
static void best_floating_line (short *values, int num_values, double *initial_y, double *final_y, short *max_error)
{
