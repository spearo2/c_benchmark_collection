# 1 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c"
...
# 1 "<command-line>" 2
# 1 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c"
# 19 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c"
...
# 20 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c" 2
...
# 21 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c" 2
...
# 23 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c" 2
# 32 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c"
...
WavpackContext *WavpackOpenFileInputEx64 (WavpackStreamReader64 *reader, void *wv_id, void *wvc_id, char *error, int flags, int norm_offset)
{
    ...
    if (!wpc) {
        ...
        return 
# 44 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c" 3 4
              ((void *)0)
# 44 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c"
                  ;
    }
    ...
    if ((flags & (0x2 | 0x40)) && wpc->reader->can_seek (wpc->wv_in)) {
        load_tag (wpc);
        wpc->reader->set_pos_abs (wpc->wv_in, 0);

        if ((flags & 0x40) && !editable_tag (&wpc->m_tag)) {
    ...
    while (!wps->wphdr.block_samples) {
        ...
        if (!WavpackVerifySingleBlock (wps->blockbuff, !(flags & 0x800))) {
            wps->wphdr.block_samples = 0;
            free (wps->blockbuff);
            wps->blockbuff = 
# 127 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c" 3 4
                            ((void *)0)
# 127 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c"
                                ;
            wpc->crc_errors++;
            continue;
        }
        ...
        if (wps->wphdr.block_samples) {
            if (flags & 0x20)
                do { int64_t tmp = (0); (wps->wphdr).block_index = (uint32_t) tmp; (wps->wphdr).block_index_u8 = (unsigned char) (tmp >> 32); } while (0);
            else if (wpc->total_samples == -1) {
                if (( (int64_t) (wps->wphdr).block_index + ((int64_t) (wps->wphdr).block_index_u8 << 32) ) || ( ((wps->wphdr).total_samples == (uint32_t) -1) ? -1 : (int64_t) (wps->wphdr).total_samples + ((int64_t) (wps->wphdr).total_samples_u8 << 32) - (wps->wphdr).total_samples_u8 ) == -1) {
                    wpc->initial_index = ( (int64_t) (wps->wphdr).block_index + ((int64_t) (wps->wphdr).block_index_u8 << 32) );
                    do { int64_t tmp = (0); (wps->wphdr).block_index = (uint32_t) tmp; (wps->wphdr).block_index_u8 = (unsigned char) (tmp >> 32); } while (0);

                    if (wpc->reader->can_seek (wpc->wv_in)) {
                        int64_t final_index = -1;

                        seek_eof_information (wpc, &final_index, 0);
                    ...
                    wpc->total_samples = ( ((wps->wphdr).total_samples == (uint32_t) -1) ? -1 : (int64_t) (wps->wphdr).total_samples + ((int64_t) (wps->wphdr).total_samples_u8 << 32) - (wps->wphdr).total_samples_u8 );
            }
        }
        else if (wpc->total_samples == -1 && !( (int64_t) (wps->wphdr).block_index + ((int64_t) (wps->wphdr).block_index_u8 << 32) ) && ( ((wps->wphdr).total_samples == (uint32_t) -1) ? -1 : (int64_t) (wps->wphdr).total_samples + ((int64_t) (wps->wphdr).total_samples_u8 << 32) - (wps->wphdr).total_samples_u8 ))
            wpc->total_samples = ( ((wps->wphdr).total_samples == (uint32_t) -1) ? -1 : (int64_t) (wps->wphdr).total_samples + ((int64_t) (wps->wphdr).total_samples_u8 << 32) - (wps->wphdr).total_samples_u8 );

        if (wpc->wvc_in && wps->wphdr.block_samples && (wps->wphdr.flags & 8)) {
            unsigned char ch;
    ...
    if (!wpc->config.num_channels) {
        wpc->config.num_channels = (wps->wphdr.flags & 4) ? 1 : 2;
        wpc->config.channel_mask = 0x5 - wpc->config.num_channels;
    }

    if ((flags & 0x8) && !(wps->wphdr.flags & 0x1000))
        wpc->reduced_channels = (wps->wphdr.flags & 4) ? 1 : 2;

    if (wps->wphdr.flags & 0x80000000) {
        ...
        wpc->config.bytes_per_sample = (wps->wphdr.flags & 3) + 1;
        wpc->config.float_norm_exp = wps->float_norm_exp;

        wpc->config.bits_per_sample = (wpc->config.bytes_per_sample * 8) -
            ((wps->wphdr.flags & (0x1fL << 13)) >> 13);
    }

    if (!wpc->config.sample_rate) {
        if (!wps->wphdr.block_samples || (wps->wphdr.flags & (0xfL << 23)) == (0xfL << 23))
            wpc->config.sample_rate = 44100;
        else
            wpc->config.sample_rate = sample_rates [(wps->wphdr.flags & (0xfL << 23)) >> 23];
    }
...
# 283 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c"
...
int unpack_init (WavpackContext *wpc)
{
    ...
    wps->dsd.ready = 0;
    memset (&wps->wvbits, 0, sizeof (wps->wvbits));;
    memset (&wps->wvcbits, 0, sizeof (wps->wvcbits));;
    memset (&wps->wvxbits, 0, sizeof (wps->wvxbits));;
    memset (&wps->decorr_passes, 0, sizeof (wps->decorr_passes));;
    memset (&wps->dc, 0, sizeof (wps->dc));;
    memset (&wps->w, 0, sizeof (wps->w));;

    if (!(wps->wphdr.flags & 4) && wpc->config.num_channels && wps->wphdr.block_samples &&
        (wpc->reduced_channels == 1 || wpc->config.num_channels == 1)) {
            wps->mute_error = 1;
            return 0;
    }

    if ((wps->wphdr.flags & 0x00000000) || (wps->wphdr.flags & (4 | 0x40000000)) == (4 | 0x40000000)) {
        wps->mute_error = 1;
        return 0;
    }
    ...
    if (wps->wphdr.block_samples && ((wps->wphdr.flags & 0x80000000) ? !wps->dsd.ready : !((&wps->wvbits)->ptr != 
# 333 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c" 3 4
                                                                                       ((void *)0)
# 333 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c"
                                                                                       ))) {
        if (((&wps->wvcbits)->ptr != 
# 334 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c" 3 4
           ((void *)0)
# 334 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c"
           ))
    ...
    if (wps->wphdr.block_samples && !((&wps->wvxbits)->ptr != 
# 341 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c" 3 4
                                    ((void *)0)
# 341 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c"
                                    )) {
        if ((wps->wphdr.flags & 0x100) && wps->int32_sent_bits)
            wpc->lossy_blocks = 1;

        if ((wps->wphdr.flags & 0x80) &&
            wps->float_flags & (0x20 | 8 | 4 | 2))
                wpc->lossy_blocks = 1;
    }

    if (wps->wphdr.block_samples)
        wps->sample_index = ( (int64_t) (wps->wphdr).block_index + ((int64_t) (wps->wphdr).block_index_u8 << 32) );

    return 1;
}
# 364 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c"
...
# 392 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c"
...
static int read_config_info (WavpackContext *wpc, WavpackMetadata *wpmd)
{
    ...
    if (bytecnt >= 3) {
        ...
        if (bytecnt && (wpc->config.flags & 0x2000000)) {
...
static int read_new_config_info (WavpackContext *wpc, WavpackMetadata *wpmd)
{
    ...
    if (wpc->channel_reordering) {
        ...
        wpc->channel_reordering = 
# 558 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c" 3 4
                                 ((void *)0)
# 558 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c"
                                     ;
    }
...
{
    if ((wpc->open_flags & 0x4) && wpc->wrapper_bytes < 16777216 && wpmd->byte_length) {
...
static int read_metadata_buff (WavpackMetadata *wpmd, unsigned char *blockbuff, unsigned char **buffptr)
{
    ...
    if (wpmd->byte_length) {
        if (buffend - *buffptr < wpmd->byte_length + (wpmd->byte_length & 1)) {
            wpmd->data = 
# 685 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c" 3 4
                        ((void *)0)
# 685 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c"
                            ;
            return 0;
        }
        ...
        wpmd->data = 
# 693 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c" 3 4
                    ((void *)0)
# 693 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c"
                        ;
...
static int process_metadata (WavpackContext *wpc, WavpackMetadata *wpmd)
{
    ...
    switch (wpmd->id) {
        ...
        case (0x20 | 0x3): case (0x20 | 0x4):
            if (!(wpc->open_flags & 0x400))
                return 1;

        case (0x20 | 0x1): case (0x20 | 0x2):
...
# 853 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c"
void WavpackSeekTrailingWrapper (WavpackContext *wpc)
{
    if ((wpc->open_flags & 0x4) &&
        wpc->reader->can_seek (wpc->wv_in) && !wpc->stream3)
            seek_eof_information (wpc, 
# 857 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c" 3 4
                                      ((void *)0)
# 857 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c"
                                          , 1);
}
...
int WavpackGetMD5Sum (WavpackContext *wpc, unsigned char data [16])
{
    if (wpc->config.flags & 0x8000000) {
        if (!wpc->config.md5_read && wpc->reader->can_seek (wpc->wv_in))
            seek_eof_information (wpc, 
# 868 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c" 3 4
                                      ((void *)0)
# 868 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c"
                                          , 0);
...
uint32_t read_next_header (WavpackStreamReader64 *reader, void *id, WavpackHeader *wphdr)
{
    ...
    while (1) {
        ...
        if (*sp++ == 'w' && *sp == 'v' && *++sp == 'p' && *++sp == 'k' &&
            ...
            sp [4] >= (0x402 & 0xff) && sp [4] <= (0x410 & 0xff) && sp [18] < 3 && !sp [19]) {
                memcpy (wphdr, buffer, sizeof (*wphdr));
                WavpackLittleEndianToNative (wphdr, "4LS2LLLLL");
...
# 928 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c"
static int match_wvc_header (WavpackHeader *wv_hdr, WavpackHeader *wvc_hdr)
{
    if (( (int64_t) (*wv_hdr).block_index + ((int64_t) (*wv_hdr).block_index_u8 << 32) ) == ( (int64_t) (*wvc_hdr).block_index + ((int64_t) (*wvc_hdr).block_index_u8 << 32) ) &&
        wv_hdr->block_samples == wvc_hdr->block_samples) {
            ...
            if (wv_hdr->flags & 0x800)
                wvi -= 1;

            if (wv_hdr->flags & 0x1000)
                wvi += 1;

            if (wvc_hdr->flags & 0x800)
                wvci -= 1;

            if (wvc_hdr->flags & 0x1000)
                wvci += 1;

            return (wvci - wvi < 0) ? 1 : -1;
        }

    if (((( (int64_t) (*wvc_hdr).block_index + ((int64_t) (*wvc_hdr).block_index_u8 << 32) ) - ( (int64_t) (*wv_hdr).block_index + ((int64_t) (*wv_hdr).block_index_u8 << 32) )) << 24) < 0)
        return 1;
    else
        return -1;
}
# 966 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c"
int read_wvc_block (WavpackContext *wpc)
{
    WavpackStream *wps = wpc->streams [wpc->current_stream];
    ...
    while (1) {
        ...
        if (wpc->open_flags & 0x20)
            do { int64_t tmp = (wps->sample_index = 0); (wphdr).block_index = (uint32_t) tmp; (wphdr).block_index_u8 = (unsigned char) (tmp >> 32); } while (0);
        else
            do { int64_t tmp = (( (int64_t) (wphdr).block_index + ((int64_t) (wphdr).block_index_u8 << 32) ) - wpc->initial_index); (wphdr).block_index = (uint32_t) tmp; (wphdr).block_index_u8 = (unsigned char) (tmp >> 32); } while (0);

        if (wphdr.flags & 0x800)
         ...
         return 0;
            ...
            if (wpc->reader->read_bytes (wpc->wvc_in, wps->block2buff + 32, wphdr.ckSize - 24) !=
                wphdr.ckSize - 24) {
                    free (wps->block2buff);
                    wps->block2buff = 
# 1004 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c" 3 4
                                     ((void *)0)
# 1004 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c"
                                         ;
                    wps->wvc_skip = 1;
                    wpc->crc_errors++;
                    return 0;
            ...
            if (!WavpackVerifySingleBlock (wps->block2buff, !(wpc->open_flags & 0x800))) {
                free (wps->block2buff);
                wps->block2buff = 
# 1015 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c" 3 4
                                 ((void *)0)
# 1015 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c"
                                     ;
                wps->wvc_skip = 1;
                wpc->crc_errors++;
                return 1;
            ...
            wps->wvc_skip = 1;
            wpc->reader->set_pos_rel (wpc->wvc_in, -32, 
# 1028 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c" 3 4
                                                       1
# 1028 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c"
                                                               );
            wpc->crc_errors++;
            return 1;
        }
...
# 1047 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c"
static int seek_eof_information (WavpackContext *wpc, int64_t *final_index, int get_wrapper)
{
    int64_t restore_pos, last_pos = -1;
    WavpackStreamReader64 *reader = wpc->reader;
    int alt_types = wpc->open_flags & 0x400;
    ...
    if (reader->get_length (id) > (int64_t) 1048576)
        reader->set_pos_rel (id, -1048576, 
# 1061 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c" 3 4
                                          2
# 1061 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c"
                                                  );
    else
        ...
        if (bcount == (uint32_t) -1) {
            ...
            if (!blocks) {
                if (current_pos > (int64_t) 2000000)
                    reader->set_pos_rel (id, -2000000, 
# 1093 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c" 3 4
                                                      1
# 1093 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c"
                                                              );
                else
                    reader->set_pos_abs (id, 0);
        ...
        if (wphdr.block_samples) {
            if (final_index)
                *final_index = ( (int64_t) (wphdr).block_index + ((int64_t) (wphdr).block_index_u8 << 32) ) + wphdr.block_samples;
            ...
            if (current_pos > (int64_t) 1048576)
                reader->set_pos_rel (id, -1048576, 
# 1121 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c" 3 4
                                                  1
# 1121 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c"
                                                          );
            else
                reader->set_pos_abs (id, 0);
        ...
        while (bcount >= 2) {
            ...
            meta_size = (meta_id & 0x40) ? meta_bc - 1 : meta_bc;
            meta_id &= 0x3f;

            if (get_wrapper && (meta_id == (0x20 | 0x2) || (alt_types && meta_id == (0x20 | 0x4))) && meta_bc) {
            ...
            else if (meta_id == (0x20 | 0x6) || (alt_types && meta_id == (0x20 | 0x9))) {
                if (meta_bc == 16 && bcount >= 16) {
                    ...
                    reader->set_pos_rel (id, meta_bc, 
# 1185 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c" 3 4
                                                     1
# 1185 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c"
                                                             );
            }
            else
                reader->set_pos_rel (id, meta_bc, 
# 1188 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c" 3 4
                                                 1
# 1188 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c"
                                                         );
...
# 1202 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c"
int WavpackVerifySingleBlock (unsigned char *buffer, int verify_checksum)
{
    ...
    while (bcount >= 2) {
        ...
        if (verify_checksum && (meta_id & 0x3f) == (0x20 | 0xf)) {
            ...
            if ((meta_id & 0x40) || meta_bc < 2 || meta_bc > 4)
                return 0;


            while (wcount--)
                csum = (csum * 3) + *csptr++;
# 1260 "/doner/wavpack/wavpack-5.1.0/src/open_utils.c"
            if (meta_bc == 4) {
    ...
    return (bcount == 0) && (!verify_checksum || !(wphdr->flags & 0x10000000) || checksum_passed);
}
