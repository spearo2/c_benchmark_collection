# 1 "/doner/wavpack/wavpack-5.1.0/src/pack_dsd.c"
...
# 1 "<command-line>" 2
# 1 "/doner/wavpack/wavpack-5.1.0/src/pack_dsd.c"
# 15 "/doner/wavpack/wavpack-5.1.0/src/pack_dsd.c"
...
# 16 "/doner/wavpack/wavpack-5.1.0/src/pack_dsd.c" 2
...
# 17 "/doner/wavpack/wavpack-5.1.0/src/pack_dsd.c" 2
...
# 18 "/doner/wavpack/wavpack-5.1.0/src/pack_dsd.c" 2
...
# 20 "/doner/wavpack/wavpack-5.1.0/src/pack_dsd.c" 2
...
# 53 "/doner/wavpack/wavpack-5.1.0/src/pack_dsd.c"
...
int pack_dsd_block (WavpackContext *wpc, int32_t *buffer)
{
    ...
    if (wpc->metacount) {
        ...
        free (wpc->metadata);
        wpc->metadata = 
# 100 "/doner/wavpack/wavpack-5.1.0/src/pack_dsd.c" 3 4
                       ((void *)0)
# 100 "/doner/wavpack/wavpack-5.1.0/src/pack_dsd.c"
                           ;
    }
    ...
    *dsd_encoding++ = dsd_power;
    ...
    if (res == -1) {
        int num_samples = sample_count * ((flags & (4 | 0x40000000)) ? 1 : 2);
...
static int rle_encode (unsigned char *src, int bcount, unsigned char *destination)
{
    ...
    while (bcount--) {
        if (*src) {
            while (zcount) {
                *dp++ = 0xa0 + (zcount > max_rle_zeros ? max_rle_zeros : zcount);
...
static void calculate_probabilities (int hist [256], unsigned char probs [256], unsigned short prob_sums [256])
{
    ...
    if (max_hits > 0xa0)
        divisor = ((max_hits << 8) + (0xa0 >> 1)) / 0xa0;
    else
        divisor = 0;
...
# 279 "/doner/wavpack/wavpack-5.1.0/src/pack_dsd.c"
        break;
    }
}
...
# 463 "/doner/wavpack/wavpack-5.1.0/src/pack_dsd.c"
...
static int normalize_ptable (int *ptable)
{
    ...
    init_ptable (ntable, rate, 20);

    for (min_error = i = 0; i < (1<<8); ++i)
        min_error += abs (ptable [i] - ntable [i]) >> 8;

    while (1) {
        init_ptable (ntable, ++rate, 20);

        for (error_sum = i = 0; i < (1<<8); ++i)
...
static int encode_buffer_high (WavpackStream *wps, int32_t *buffer, int num_samples, unsigned char *destination)
{
    int channel, stereo = (wps->wphdr.flags & (4 | 0x40000000)) ? 0 : 1;
    ...
    *dp++ = 3;
    ...
    if (!wps->sample_index) {
        ...
        init_ptable (wps->dsd.ptable, (1536/(1<<8)), 20);

        for (channel = 0; channel < 2; ++channel) {
            sp = wps->dsd.filters + channel;

            sp->filter1 = sp->filter2 = sp->filter3 = sp->filter4 = sp->filter5 = (1 << 20) / 2;
        ...
        *dp++ = 20;
        ...
        init_ptable (wps->dsd.ptable, rate, 20);
        *dp++ = rate;
        *dp++ = 20;
    }
