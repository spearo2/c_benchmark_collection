# 1 "/doner/wavpack/wavpack-5.1.0/src/read_words.c"
...
# 1 "<command-line>" 2
# 1 "/doner/wavpack/wavpack-5.1.0/src/read_words.c"
# 27 "/doner/wavpack/wavpack-5.1.0/src/read_words.c"
...
# 28 "/doner/wavpack/wavpack-5.1.0/src/read_words.c" 2
...
# 29 "/doner/wavpack/wavpack-5.1.0/src/read_words.c" 2
...
# 31 "/doner/wavpack/wavpack-5.1.0/src/read_words.c" 2
# 57 "/doner/wavpack/wavpack-5.1.0/src/read_words.c"
static uint32_t __inline read_code (Bitstream *bs, uint32_t maxcode);
# 67 "/doner/wavpack/wavpack-5.1.0/src/read_words.c"
int32_t get_word (WavpackStream *wps, int chan, int32_t *correction)
{
    ...
    if (!(wps->w.c [0].median [0] & ~1) && !wps->w.holding_zero && !wps->w.holding_one && !(wps->w.c [1].median [0] & ~1)) {
        ...
        if (wps->w.zeros_acc) {
            ...
            for (cbits = 0; cbits < 33 && ( (((&wps->wvbits)->bc) ? ((&wps->wvbits)->bc--, (&wps->wvbits)->sr & 1) : (((++((&wps->wvbits)->ptr) != (&wps->wvbits)->end) ? (void) 0 : (&wps->wvbits)->wrap (&wps->wvbits)), (&wps->wvbits)->bc = sizeof (*((&wps->wvbits)->ptr)) * 8 - 1, ((&wps->wvbits)->sr = *((&wps->wvbits)->ptr)) & 1) ) ? ((&wps->wvbits)->sr >>= 1, 1) : ((&wps->wvbits)->sr >>= 1, 0) ); ++cbits);
            ...
            if (cbits < 2)
                ...
                for (mask = 1, wps->w.zeros_acc = 0; --cbits; mask <<= 1)
                    if (( (((&wps->wvbits)->bc) ? ((&wps->wvbits)->bc--, (&wps->wvbits)->sr & 1) : (((++((&wps->wvbits)->ptr) != (&wps->wvbits)->end) ? (void) 0 : (&wps->wvbits)->wrap (&wps->wvbits)), (&wps->wvbits)->bc = sizeof (*((&wps->wvbits)->ptr)) * 8 - 1, ((&wps->wvbits)->sr = *((&wps->wvbits)->ptr)) & 1) ) ? ((&wps->wvbits)->sr >>= 1, 1) : ((&wps->wvbits)->sr >>= 1, 0) ))
    ...
    if (wps->w.holding_zero)
        ...
        if (ones_count >= 16) {
            ...
            for (; ones_count < (16 + 1) && ( (((&wps->wvbits)->bc) ? ((&wps->wvbits)->bc--, (&wps->wvbits)->sr & 1) : (((++((&wps->wvbits)->ptr) != (&wps->wvbits)->end) ? (void) 0 : (&wps->wvbits)->wrap (&wps->wvbits)), (&wps->wvbits)->bc = sizeof (*((&wps->wvbits)->ptr)) * 8 - 1, ((&wps->wvbits)->sr = *((&wps->wvbits)->ptr)) & 1) ) ? ((&wps->wvbits)->sr >>= 1, 1) : ((&wps->wvbits)->sr >>= 1, 0) ); ++ones_count);
...
# 245 "/doner/wavpack/wavpack-5.1.0/src/read_words.c"
    ...
    sign = ( (((&wps->wvbits)->bc) ? ((&wps->wvbits)->bc--, (&wps->wvbits)->sr & 1) : (((++((&wps->wvbits)->ptr) != (&wps->wvbits)->end) ? (void) 0 : (&wps->wvbits)->wrap (&wps->wvbits)), (&wps->wvbits)->bc = sizeof (*((&wps->wvbits)->ptr)) * 8 - 1, ((&wps->wvbits)->sr = *((&wps->wvbits)->ptr)) & 1) ) ? ((&wps->wvbits)->sr >>= 1, 1) : ((&wps->wvbits)->sr >>= 1, 0) );

    if (((&wps->wvcbits)->ptr != 
# 308 "/doner/wavpack/wavpack-5.1.0/src/read_words.c" 3 4
       ((void *)0)
# 308 "/doner/wavpack/wavpack-5.1.0/src/read_words.c"
       ) && c->error_limit) {
...
int32_t get_words_lossless (WavpackStream *wps, int32_t *buffer, int32_t nsamples)
{
    ...
    if (nsamples && !bs->ptr) {
        memset (buffer, 0, (wps->wphdr.flags & (4 | 0x40000000)) ? nsamples * 4 : nsamples * 8);
        return nsamples;
    }
    ...
    for (csamples = 0; csamples < nsamples; ++csamples) {
        ...
        if (wps->w.holding_zero) {
            ...
            low = read_code (bs, (((c->median [0]) >> 4) + 1) - 1);
            (c->median [0] -= ((c->median [0] + (128 -2)) / 128) * 2);
            buffer [csamples] = (( (((bs)->bc) ? ((bs)->bc--, (bs)->sr & 1) : (((++((bs)->ptr) != (bs)->end) ? (void) 0 : (bs)->wrap (bs)), (bs)->bc = sizeof (*((bs)->ptr)) * 8 - 1, ((bs)->sr = *((bs)->ptr)) & 1) ) ? ((bs)->sr >>= 1, 1) : ((bs)->sr >>= 1, 0) )) ? ~low : low;
        ...
        if (wps->w.c [0].median [0] < 2 && !wps->w.holding_one && wps->w.c [1].median [0] < 2) {
            ...
            if (wps->w.zeros_acc) {
                ...
                for (cbits = 0; cbits < 33 && ( (((bs)->bc) ? ((bs)->bc--, (bs)->sr & 1) : (((++((bs)->ptr) != (bs)->end) ? (void) 0 : (bs)->wrap (bs)), (bs)->bc = sizeof (*((bs)->ptr)) * 8 - 1, ((bs)->sr = *((bs)->ptr)) & 1) ) ? ((bs)->sr >>= 1, 1) : ((bs)->sr >>= 1, 0) ); ++cbits);
                ...
                if (cbits < 2)
                    ...
                    for (mask = 1, wps->w.zeros_acc = 0; --cbits; mask <<= 1)
                        if (( (((bs)->bc) ? ((bs)->bc--, (bs)->sr & 1) : (((++((bs)->ptr) != (bs)->end) ? (void) 0 : (bs)->wrap (bs)), (bs)->bc = sizeof (*((bs)->ptr)) * 8 - 1, ((bs)->sr = *((bs)->ptr)) & 1) ) ? ((bs)->sr >>= 1, 1) : ((bs)->sr >>= 1, 0) ))
                            wps->w.zeros_acc |= mask;
        ...
        if (ones_count >= 16) {
            ...
            for (; ones_count < (16 + 1) && ( (((bs)->bc) ? ((bs)->bc--, (bs)->sr & 1) : (((++((bs)->ptr) != (bs)->end) ? (void) 0 : (bs)->wrap (bs)), (bs)->bc = sizeof (*((bs)->ptr)) * 8 - 1, ((bs)->sr = *((bs)->ptr)) & 1) ) ? ((bs)->sr >>= 1, 1) : ((bs)->sr >>= 1, 0) ); ++ones_count);
...
# 522 "/doner/wavpack/wavpack-5.1.0/src/read_words.c"
    ...
    return (wps->wphdr.flags & (4 | 0x40000000)) ? csamples : (csamples / 2);
}
...
static uint32_t __inline read_code (Bitstream *bs, uint32_t maxcode)
{
    ...
    if (maxcode < 2)
        return maxcode ? ( (((bs)->bc) ? ((bs)->bc--, (bs)->sr & 1) : (((++((bs)->ptr) != (bs)->end) ? (void) 0 : (bs)->wrap (bs)), (bs)->bc = sizeof (*((bs)->ptr)) * 8 - 1, ((bs)->sr = *((bs)->ptr)) & 1) ) ? ((bs)->sr >>= 1, 1) : ((bs)->sr >>= 1, 0) ) : 0;

    bitcount = ((maxcode) ? 32 - __builtin_clz (maxcode) : 0);
