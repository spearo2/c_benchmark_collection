# 1 "/doner/wavpack/wavpack-5.1.0/src/pack_floats.c"
...
# 1 "<command-line>" 2
# 1 "/doner/wavpack/wavpack-5.1.0/src/pack_floats.c"
# 16 "/doner/wavpack/wavpack-5.1.0/src/pack_floats.c"
...
# 17 "/doner/wavpack/wavpack-5.1.0/src/pack_floats.c" 2
...
# 19 "/doner/wavpack/wavpack-5.1.0/src/pack_floats.c" 2
# 31 "/doner/wavpack/wavpack-5.1.0/src/pack_floats.c"
int scan_float_data (WavpackStream *wps, f32 *values, int32_t num_values)
{
    ...
    for (dp = values, count = num_values; count--; dp++) {
        crc = crc * 27 + ((*dp) & 0x7fffff) * 9 + (((*dp) >> 23) & 0xff) * 3 + (((*dp) >> 31) & 0x1);

        if ((((*dp) >> 23) & 0xff) < 255 && ((*dp) & 0x7fffffff) > max_mag)
            max_mag = ((*dp) & 0x7fffffff);
    }
    ...
    if ((((max_mag) >> 23) & 0xff))
        max_exp = (((max_mag + 0x7F0000) >> 23) & 0xff);
    ...
    for (dp = values, count = num_values; count--; dp++) {
        ...
        if ((((*dp) >> 23) & 0xff) == 255) {
            ...
            shift_count = max_exp - (((*dp) >> 23) & 0xff);
            value = 0x800000 + ((*dp) & 0x7fffff);
        }
        ...
        ordata |= value;
        * (int32_t *) dp = ((((*dp) >> 31) & 0x1)) ? -value : value;
    }
    ...
    if (neg_zeros)
        wps->float_flags |= 0x10;
# 197 "/doner/wavpack/wavpack-5.1.0/src/pack_floats.c"
    return wps->float_flags & (0x20 | 8 | 4 | 2);
}
# 207 "/doner/wavpack/wavpack-5.1.0/src/pack_floats.c"
void send_float_data (WavpackStream *wps, f32 *values, int32_t num_values)
{
    ...
    for (dp = values, count = num_values; count--; dp++) {
        if ((((*dp) >> 23) & 0xff) == 255) {
            if (((*dp) & 0x7fffff)) {
                do { (&wps->wvxbits)->sr |= (1 << (&wps->wvxbits)->bc); if (++((&wps->wvxbits)->bc) == sizeof (*((&wps->wvxbits)->ptr)) * 8) { *((&wps->wvxbits)->ptr) = (&wps->wvxbits)->sr; (&wps->wvxbits)->sr = (&wps->wvxbits)->bc = 0; if (++((&wps->wvxbits)->ptr) == (&wps->wvxbits)->end) (&wps->wvxbits)->wrap (&wps->wvxbits); }} while (0);
                do { (&wps->wvxbits)->sr |= (int32_t)(((*dp) & 0x7fffff)) << (&wps->wvxbits)->bc; if (((&wps->wvxbits)->bc += (23)) >= sizeof (*((&wps->wvxbits)->ptr)) * 8) do { *((&wps->wvxbits)->ptr) = (&wps->wvxbits)->sr; (&wps->wvxbits)->sr >>= sizeof (*((&wps->wvxbits)->ptr)) * 8; if (((&wps->wvxbits)->bc -= sizeof (*((&wps->wvxbits)->ptr)) * 8) > 32 - sizeof (*((&wps->wvxbits)->ptr)) * 8) (&wps->wvxbits)->sr |= ((((*dp) & 0x7fffff)) >> ((23) - (&wps->wvxbits)->bc)); if (++((&wps->wvxbits)->ptr) == (&wps->wvxbits)->end) (&wps->wvxbits)->wrap (&wps->wvxbits); } while ((&wps->wvxbits)->bc >= sizeof (*((&wps->wvxbits)->ptr)) * 8); } while (0);
            }
            else {
                do { if (++((&wps->wvxbits)->bc) == sizeof (*((&wps->wvxbits)->ptr)) * 8) { *((&wps->wvxbits)->ptr) = (&wps->wvxbits)->sr; (&wps->wvxbits)->sr = (&wps->wvxbits)->bc = 0; if (++((&wps->wvxbits)->ptr) == (&wps->wvxbits)->end) (&wps->wvxbits)->wrap (&wps->wvxbits); }} while (0);
            }
        ...
        if (!value) {
            if (wps->float_flags & 8) {
                if ((((*dp) >> 23) & 0xff) || ((*dp) & 0x7fffff)) {
                    do { (&wps->wvxbits)->sr |= (1 << (&wps->wvxbits)->bc); if (++((&wps->wvxbits)->bc) == sizeof (*((&wps->wvxbits)->ptr)) * 8) { *((&wps->wvxbits)->ptr) = (&wps->wvxbits)->sr; (&wps->wvxbits)->sr = (&wps->wvxbits)->bc = 0; if (++((&wps->wvxbits)->ptr) == (&wps->wvxbits)->end) (&wps->wvxbits)->wrap (&wps->wvxbits); }} while (0);
                    do { (&wps->wvxbits)->sr |= (int32_t)(((*dp) & 0x7fffff)) << (&wps->wvxbits)->bc; if (((&wps->wvxbits)->bc += (23)) >= sizeof (*((&wps->wvxbits)->ptr)) * 8) do { *((&wps->wvxbits)->ptr) = (&wps->wvxbits)->sr; (&wps->wvxbits)->sr >>= sizeof (*((&wps->wvxbits)->ptr)) * 8; if (((&wps->wvxbits)->bc -= sizeof (*((&wps->wvxbits)->ptr)) * 8) > 32 - sizeof (*((&wps->wvxbits)->ptr)) * 8) (&wps->wvxbits)->sr |= ((((*dp) & 0x7fffff)) >> ((23) - (&wps->wvxbits)->bc)); if (++((&wps->wvxbits)->ptr) == (&wps->wvxbits)->end) (&wps->wvxbits)->wrap (&wps->wvxbits); } while ((&wps->wvxbits)->bc >= sizeof (*((&wps->wvxbits)->ptr)) * 8); } while (0);

                    if (max_exp >= 25) {
                        do { (&wps->wvxbits)->sr |= (int32_t)((((*dp) >> 23) & 0xff)) << (&wps->wvxbits)->bc; if (((&wps->wvxbits)->bc += (8)) >= sizeof (*((&wps->wvxbits)->ptr)) * 8) do { *((&wps->wvxbits)->ptr) = (&wps->wvxbits)->sr; (&wps->wvxbits)->sr >>= sizeof (*((&wps->wvxbits)->ptr)) * 8; if (((&wps->wvxbits)->bc -= sizeof (*((&wps->wvxbits)->ptr)) * 8) > 32 - sizeof (*((&wps->wvxbits)->ptr)) * 8) (&wps->wvxbits)->sr |= (((((*dp) >> 23) & 0xff)) >> ((8) - (&wps->wvxbits)->bc)); if (++((&wps->wvxbits)->ptr) == (&wps->wvxbits)->end) (&wps->wvxbits)->wrap (&wps->wvxbits); } while ((&wps->wvxbits)->bc >= sizeof (*((&wps->wvxbits)->ptr)) * 8); } while (0);
                    }

                    do { if ((((*dp) >> 31) & 0x1)) (&wps->wvxbits)->sr |= (1 << (&wps->wvxbits)->bc); if (++((&wps->wvxbits)->bc) == sizeof (*((&wps->wvxbits)->ptr)) * 8) { *((&wps->wvxbits)->ptr) = (&wps->wvxbits)->sr; (&wps->wvxbits)->sr = (&wps->wvxbits)->bc = 0; if (++((&wps->wvxbits)->ptr) == (&wps->wvxbits)->end) (&wps->wvxbits)->wrap (&wps->wvxbits); }} while (0);
                }
                else {
                    do { if (++((&wps->wvxbits)->bc) == sizeof (*((&wps->wvxbits)->ptr)) * 8) { *((&wps->wvxbits)->ptr) = (&wps->wvxbits)->sr; (&wps->wvxbits)->sr = (&wps->wvxbits)->bc = 0; if (++((&wps->wvxbits)->ptr) == (&wps->wvxbits)->end) (&wps->wvxbits)->wrap (&wps->wvxbits); }} while (0);

                    if (wps->float_flags & 0x10)
                        do { if ((((*dp) >> 31) & 0x1)) (&wps->wvxbits)->sr |= (1 << (&wps->wvxbits)->bc); if (++((&wps->wvxbits)->bc) == sizeof (*((&wps->wvxbits)->ptr)) * 8) { *((&wps->wvxbits)->ptr) = (&wps->wvxbits)->sr; (&wps->wvxbits)->sr = (&wps->wvxbits)->bc = 0; if (++((&wps->wvxbits)->ptr) == (&wps->wvxbits)->end) (&wps->wvxbits)->wrap (&wps->wvxbits); }} while (0);
                }
            }
        }
        else if (shift_count) {
            if (wps->float_flags & 4) {
                int32_t data = ((*dp) & 0x7fffff) & ((1 << shift_count) - 1);
                do { (&wps->wvxbits)->sr |= (int32_t)(data) << (&wps->wvxbits)->bc; if (((&wps->wvxbits)->bc += (shift_count)) >= sizeof (*((&wps->wvxbits)->ptr)) * 8) do { *((&wps->wvxbits)->ptr) = (&wps->wvxbits)->sr; (&wps->wvxbits)->sr >>= sizeof (*((&wps->wvxbits)->ptr)) * 8; if (((&wps->wvxbits)->bc -= sizeof (*((&wps->wvxbits)->ptr)) * 8) > 32 - sizeof (*((&wps->wvxbits)->ptr)) * 8) (&wps->wvxbits)->sr |= ((data) >> ((shift_count) - (&wps->wvxbits)->bc)); if (++((&wps->wvxbits)->ptr) == (&wps->wvxbits)->end) (&wps->wvxbits)->wrap (&wps->wvxbits); } while ((&wps->wvxbits)->bc >= sizeof (*((&wps->wvxbits)->ptr)) * 8); } while (0);
            }
            else if (wps->float_flags & 2) {
                do { if (((*dp) & 0x7fffff) & 1) (&wps->wvxbits)->sr |= (1 << (&wps->wvxbits)->bc); if (++((&wps->wvxbits)->bc) == sizeof (*((&wps->wvxbits)->ptr)) * 8) { *((&wps->wvxbits)->ptr) = (&wps->wvxbits)->sr; (&wps->wvxbits)->sr = (&wps->wvxbits)->bc = 0; if (++((&wps->wvxbits)->ptr) == (&wps->wvxbits)->end) (&wps->wvxbits)->wrap (&wps->wvxbits); }} while (0);
            }
        }
    }
}
