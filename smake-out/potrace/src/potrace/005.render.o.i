# 1 "/doner/potrace/source/src/render.c"
...
# 7 "/doner/potrace/source/src/render.c" 2
...
# 8 "/doner/potrace/source/src/render.c" 2
...
# 9 "/doner/potrace/source/src/render.c" 2
...
# 10 "/doner/potrace/source/src/render.c" 2

# 1 "/doner/potrace/source/src/render.h" 1
# 9 "/doner/potrace/source/src/render.h"
...
# 10 "/doner/potrace/source/src/render.h" 2
...
# 12 "/doner/potrace/source/src/render.c" 2
...
# 19 "/doner/potrace/source/src/auxiliary.h" 2
...
# 14 "/doner/potrace/source/src/render.c" 2
# 46 "/doner/potrace/source/src/render.c"
render_t *render_new(greymap_t *gm) {
  render_t *rm;
  ...
  if (!rm) {
    return 
# 51 "/doner/potrace/source/src/render.c" 3 4
          ((void *)0)
# 51 "/doner/potrace/source/src/render.c"
              ;
  }
  ...
  if (!rm->incrow_buf) {
    free(rm);
    return 
# 58 "/doner/potrace/source/src/render.c" 3 4
          ((void *)0)
# 58 "/doner/potrace/source/src/render.c"
              ;
  }
...
void render_close(render_t *rm) {
  ...
  (((int)(rm->x0i)>=0 && (int)(rm->x0i)<(rm->gm)->w && (int)(rm->y0i)>=0 && (int)(rm->y0i)<(rm->gm)->h) ? (*(&(rm->gm)->map[(rm->x0i)+(rm->y0i)*(rm->gm)->w]) += (short int)((rm->a0+rm->a1)*255)) : 0);
...
static void incrow(render_t *rm, int x, int y, int b) {
  ...
  if (x0 < x) {
    for (i=x0; i<x; i++) {
      (((int)(i)>=0 && (int)(i)<(rm->gm)->w && (int)(y)>=0 && (int)(y)<(rm->gm)->h) ? (*(&(rm->gm)->map[(i)+(y)*(rm->gm)->w]) += (short int)(-b)) : 0);
    }
  } else {
    for (i=x; i<x0; i++) {
      (((int)(i)>=0 && (int)(i)<(rm->gm)->w && (int)(y)>=0 && (int)(y)<(rm->gm)->h) ? (*(&(rm->gm)->map[(i)+(y)*(rm->gm)->w]) += (short int)(b)) : 0);
...
void render_lineto(render_t *rm, double x2, double y2) {
  ...
  sn = ((x2i - rm->x1i)>0 ? (x2i - rm->x1i) : -(x2i - rm->x1i));
  tn = ((y2i - rm->y1i)>0 ? (y2i - rm->y1i) : -(y2i - rm->y1i));
  ...
  while (i<sn || j<tn) {
    ...
    if (s && x2>rm->x1) {
      (((int)(rxi)>=0 && (int)(rxi)<(rm->gm)->w && (int)(ryi)>=0 && (int)(ryi)<(rm->gm)->h) ? (*(&(rm->gm)->map[(rxi)+(ryi)*(rm->gm)->w]) += (short int)(rm->a1*255)) : 0);
      rm->a1 = 0;
      rxi++;
...
void render_curveto(render_t *rm, double x2, double y2, double x3, double y3, double x4, double y4) {
  ...
  dd0 = ((x1-2*x2+x3)*(x1-2*x2+x3)) + ((y1-2*y2+y3)*(y1-2*y2+y3));
  dd1 = ((x2-2*x3+x4)*(x2-2*x3+x4)) + ((y2-2*y3+y4)*(y2-2*y3+y4));
  dd = 6*sqrt(((dd0)>(dd1) ? (dd0) : (dd1)));
  e2 = 8*delta <= dd ? 8*delta/dd : 1;
  epsilon = sqrt(e2);

  for (t=epsilon; t<1; t+=epsilon) {
    render_lineto(rm, x1*((1-t)*(1-t)*(1-t))+3*x2*((1-t)*(1-t))*t+3*x3*(1-t)*((t)*(t))+x4*((t)*(t)*(t)),
    y1*((1-t)*(1-t)*(1-t))+3*y2*((1-t)*(1-t))*t+3*y3*(1-t)*((t)*(t))+y4*((t)*(t)*(t)));
  }
