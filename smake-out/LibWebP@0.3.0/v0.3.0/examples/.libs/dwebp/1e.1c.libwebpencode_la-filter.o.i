# 1 "/doner/libwebp/libwebp-v0.3.0/src/enc/filter.c"
...
# 1 "<command-line>" 2
# 1 "/doner/libwebp/libwebp-v0.3.0/src/enc/filter.c"
# 12 "/doner/libwebp/libwebp-v0.3.0/src/enc/filter.c"
...
# 13 "/doner/libwebp/libwebp-v0.3.0/src/enc/filter.c" 2
...
static void DoFilter(const VP8EncIterator* const it, int level) {
  ...
  uint8_t* const u_dst = it->yuv_out2_ + ((16 * 16));
  uint8_t* const v_dst = it->yuv_out2_ + (((16 * 16)) + 8);


  memcpy(y_dst, it->yuv_out_, ((16 * 16) + (16 * 8)) * sizeof(uint8_t));
...
enum { KERNEL = 3 };
static const double kMinValue = 1.e-10;
...
static double GetMBSSIM(const uint8_t* yuv1, const uint8_t* yuv2) {
  ...
  for (x = 1; x < 7; x++) {
    for (y = 1; y < 7; y++) {
      VP8SSIMAccumulate(yuv1 + ((16 * 16)), 16, yuv2 + ((16 * 16)), 16, x, y, 8, 8, &s);
      VP8SSIMAccumulate(yuv1 + (((16 * 16)) + 8), 16, yuv2 + (((16 * 16)) + 8), 16, x, y, 8, 8, &s);
    }
  }
  return VP8SSIMGet(&s);
}
...
void VP8StoreFilterStats(VP8EncIterator* const it) {
  ...
  const int level0 = it->enc_->dqm_[s].fstrength_;
