# 1 "/doner/libwebp/libwebp-v0.3.0/src/enc/iterator.c"
...
# 1 "<command-line>" 2
# 1 "/doner/libwebp/libwebp-v0.3.0/src/enc/iterator.c"
# 12 "/doner/libwebp/libwebp-v0.3.0/src/enc/iterator.c"
...
# 13 "/doner/libwebp/libwebp-v0.3.0/src/enc/iterator.c" 2
...
# 15 "/doner/libwebp/libwebp-v0.3.0/src/enc/iterator.c" 2
# 24 "/doner/libwebp/libwebp-v0.3.0/src/enc/iterator.c"
...
void VP8IteratorImport(const VP8EncIterator* const it) {
  ...
  uint8_t* const ydst = it->yuv_in_ + (0);
  uint8_t* const udst = it->yuv_in_ + ((16 * 16));
  uint8_t* const vdst = it->yuv_in_ + (((16 * 16)) + 8);
...
void VP8IteratorExport(const VP8EncIterator* const it) {
  ...
  if (enc->config_->show_compressed) {
    ...
    const uint8_t* const usrc = it->yuv_out_ + ((16 * 16));
    const uint8_t* const vsrc = it->yuv_out_ + (((16 * 16)) + 8);
    const WebPPicture* const pic = enc->pic_;
...
# 189 "/doner/libwebp/libwebp-v0.3.0/src/enc/iterator.c"
void VP8IteratorNzToBytes(VP8EncIterator* const it) {
...
int VP8IteratorNext(VP8EncIterator* const it,
  ...
  if (block_to_save) {
    const int x = it->x_, y = it->y_;
    const uint8_t* const ysrc = block_to_save + (0);
    const uint8_t* const usrc = block_to_save + ((16 * 16));
    if (x < enc->mb_w_ - 1) {
      int i;
      for (i = 0; i < 16; ++i) {
    ...
    if (y < enc->mb_h_ - 1) {
      memcpy(enc->y_top_ + x * 16, ysrc + 15 * 16, 16);
      memcpy(enc->uv_top_ + x * 16, usrc + 7 * 16, 8 + 8);
    }
  }
...
# 355 "/doner/libwebp/libwebp-v0.3.0/src/enc/iterator.c"
