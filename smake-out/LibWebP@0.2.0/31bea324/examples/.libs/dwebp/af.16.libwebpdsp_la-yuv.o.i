# 1 "/doner/libwebp/libwebp-31bea324/src/dsp/yuv.c"
...
# 1 "<command-line>" 2
# 1 "/doner/libwebp/libwebp-31bea324/src/dsp/yuv.c"
# 14 "/doner/libwebp/libwebp-31bea324/src/dsp/yuv.c"
# 1 "../../src/dsp/yuv.h" 1
# 38 "../../src/dsp/yuv.h"
# 1 "../../src/dsp/dsp.h" 1
# 18 "../../src/dsp/dsp.h"
...
# 15 "/doner/libwebp/libwebp-31bea324/src/dsp/yuv.c" 2

# 1 "/usr/include/assert.h" 1 3 4
# 17 "/doner/libwebp/libwebp-31bea324/src/dsp/yuv.c" 2
...
# 18 "/doner/libwebp/libwebp-31bea324/src/dsp/yuv.c" 2
# 41 "/doner/libwebp/libwebp-31bea324/src/dsp/yuv.c"

# 41 "/doner/libwebp/libwebp-31bea324/src/dsp/yuv.c"
static void YuvToRgbRow(const uint8_t* y, const uint8_t* u, const uint8_t* v, uint8_t* dst, int len) { const uint8_t* const end = dst + (len & ~1) * (3); while (dst != end) { VP8YuvToRgb(y[0], u[0], v[0], dst); VP8YuvToRgb(y[1], u[0], v[0], dst + (3)); y += 2; ++u; ++v; dst += 2 * (3); } if (len & 1) { VP8YuvToRgb(y[0], u[0], v[0], dst); } }
static void YuvToBgrRow(const uint8_t* y, const uint8_t* u, const uint8_t* v, uint8_t* dst, int len) { const uint8_t* const end = dst + (len & ~1) * (3); while (dst != end) { VP8YuvToBgr(y[0], u[0], v[0], dst); VP8YuvToBgr(y[1], u[0], v[0], dst + (3)); y += 2; ++u; ++v; dst += 2 * (3); } if (len & 1) { VP8YuvToBgr(y[0], u[0], v[0], dst); } }
static void YuvToRgbaRow(const uint8_t* y, const uint8_t* u, const uint8_t* v, uint8_t* dst, int len) { const uint8_t* const end = dst + (len & ~1) * (4); while (dst != end) { VP8YuvToRgba(y[0], u[0], v[0], dst); VP8YuvToRgba(y[1], u[0], v[0], dst + (4)); y += 2; ++u; ++v; dst += 2 * (4); } if (len & 1) { VP8YuvToRgba(y[0], u[0], v[0], dst); } }
static void YuvToBgraRow(const uint8_t* y, const uint8_t* u, const uint8_t* v, uint8_t* dst, int len) { const uint8_t* const end = dst + (len & ~1) * (4); while (dst != end) { VP8YuvToBgra(y[0], u[0], v[0], dst); VP8YuvToBgra(y[1], u[0], v[0], dst + (4)); y += 2; ++u; ++v; dst += 2 * (4); } if (len & 1) { VP8YuvToBgra(y[0], u[0], v[0], dst); } }
static void YuvToArgbRow(const uint8_t* y, const uint8_t* u, const uint8_t* v, uint8_t* dst, int len) { const uint8_t* const end = dst + (len & ~1) * (4); while (dst != end) { VP8YuvToArgb(y[0], u[0], v[0], dst); VP8YuvToArgb(y[1], u[0], v[0], dst + (4)); y += 2; ++u; ++v; dst += 2 * (4); } if (len & 1) { VP8YuvToArgb(y[0], u[0], v[0], dst); } }
static void YuvToRgba4444Row(const uint8_t* y, const uint8_t* u, const uint8_t* v, uint8_t* dst, int len) { const uint8_t* const end = dst + (len & ~1) * (2); while (dst != end) { VP8YuvToRgba4444(y[0], u[0], v[0], dst); VP8YuvToRgba4444(y[1], u[0], v[0], dst + (2)); y += 2; ++u; ++v; dst += 2 * (2); } if (len & 1) { VP8YuvToRgba4444(y[0], u[0], v[0], dst); } }
static void YuvToRgb565Row(const uint8_t* y, const uint8_t* u, const uint8_t* v, uint8_t* dst, int len) { const uint8_t* const end = dst + (len & ~1) * (2); while (dst != end) { VP8YuvToRgb565(y[0], u[0], v[0], dst); VP8YuvToRgb565(y[1], u[0], v[0], dst + (2)); y += 2; ++u; ++v; dst += 2 * (2); } if (len & 1) { VP8YuvToRgb565(y[0], u[0], v[0], dst); } }
...
static void WebPInitSamplers_body(void); void WebPInitSamplers(void) { do { static volatile VP8CPUInfo WebPInitSamplers_body_last_cpuinfo_used = (VP8CPUInfo)&WebPInitSamplers_body_last_cpuinfo_used; static pthread_mutex_t WebPInitSamplers_body_lock = 
# 79 "/doner/libwebp/libwebp-31bea324/src/dsp/yuv.c" 3 4
{ { 0, 0, 0, 0, PTHREAD_MUTEX_TIMED_NP, 0, 0, { 0, 0 } } }
# 79 "/doner/libwebp/libwebp-31bea324/src/dsp/yuv.c"
; if (pthread_mutex_lock(&WebPInitSamplers_body_lock)) break; if (WebPInitSamplers_body_last_cpuinfo_used != VP8GetCPUInfo) WebPInitSamplers_body(); WebPInitSamplers_body_last_cpuinfo_used = VP8GetCPUInfo; (void)pthread_mutex_unlock(&WebPInitSamplers_body_lock); } while (0); } static void WebPInitSamplers_body(void) {
  ...
  if (VP8GetCPUInfo != 
# 93 "/doner/libwebp/libwebp-31bea324/src/dsp/yuv.c" 3 4
                      ((void *)0)
# 93 "/doner/libwebp/libwebp-31bea324/src/dsp/yuv.c"
                          ) {
...
# 114 "/doner/libwebp/libwebp-31bea324/src/dsp/yuv.c"
  }
}
...
static void WebPInitConvertARGBToYUV_body(void); void WebPInitConvertARGBToYUV(void) { do { static volatile VP8CPUInfo WebPInitConvertARGBToYUV_body_last_cpuinfo_used = (VP8CPUInfo)&WebPInitConvertARGBToYUV_body_last_cpuinfo_used; static pthread_mutex_t WebPInitConvertARGBToYUV_body_lock = 
# 211 "/doner/libwebp/libwebp-31bea324/src/dsp/yuv.c" 3 4
{ { 0, 0, 0, 0, PTHREAD_MUTEX_TIMED_NP, 0, 0, { 0, 0 } } }
# 211 "/doner/libwebp/libwebp-31bea324/src/dsp/yuv.c"
; if (pthread_mutex_lock(&WebPInitConvertARGBToYUV_body_lock)) break; if (WebPInitConvertARGBToYUV_body_last_cpuinfo_used != VP8GetCPUInfo) WebPInitConvertARGBToYUV_body(); WebPInitConvertARGBToYUV_body_last_cpuinfo_used = VP8GetCPUInfo; (void)pthread_mutex_unlock(&WebPInitConvertARGBToYUV_body_lock); } while (0); } static void WebPInitConvertARGBToYUV_body(void) {
  ...
  if (VP8GetCPUInfo != 
# 220 "/doner/libwebp/libwebp-31bea324/src/dsp/yuv.c" 3 4
                      ((void *)0)
# 220 "/doner/libwebp/libwebp-31bea324/src/dsp/yuv.c"
                          ) {
...
# 240 "/doner/libwebp/libwebp-31bea324/src/dsp/yuv.c"
  
# 240 "/doner/libwebp/libwebp-31bea324/src/dsp/yuv.c" 3 4
 ((void) (0))
# 240 "/doner/libwebp/libwebp-31bea324/src/dsp/yuv.c"
                                   ;
  
# 241 "/doner/libwebp/libwebp-31bea324/src/dsp/yuv.c" 3 4
 ((void) (0))
# 241 "/doner/libwebp/libwebp-31bea324/src/dsp/yuv.c"
                                    ;
  
# 242 "/doner/libwebp/libwebp-31bea324/src/dsp/yuv.c" 3 4
 ((void) (0))
# 242 "/doner/libwebp/libwebp-31bea324/src/dsp/yuv.c"
                                    ;
  
# 243 "/doner/libwebp/libwebp-31bea324/src/dsp/yuv.c" 3 4
 ((void) (0))
# 243 "/doner/libwebp/libwebp-31bea324/src/dsp/yuv.c"
                                    ;
  
# 244 "/doner/libwebp/libwebp-31bea324/src/dsp/yuv.c" 3 4
 ((void) (0))
# 244 "/doner/libwebp/libwebp-31bea324/src/dsp/yuv.c"
                                      ;
}
