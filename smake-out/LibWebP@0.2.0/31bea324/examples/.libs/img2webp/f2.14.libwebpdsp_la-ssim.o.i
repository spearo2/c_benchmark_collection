# 1 "/doner/libwebp/libwebp-31bea324/src/dsp/ssim.c"
...
# 1 "<command-line>" 2
# 1 "/doner/libwebp/libwebp-31bea324/src/dsp/ssim.c"
# 14 "/doner/libwebp/libwebp-31bea324/src/dsp/ssim.c"
...
# 15 "/doner/libwebp/libwebp-31bea324/src/dsp/ssim.c" 2
...
# 16 "/doner/libwebp/libwebp-31bea324/src/dsp/ssim.c" 2
...
# 18 "/doner/libwebp/libwebp-31bea324/src/dsp/ssim.c" 2
...
static const uint32_t kWeight[2 * 3 + 1] = {
  1, 2, 3, 4, 3, 2, 1
};
static const uint32_t kWeightSum = 16 * 16;

static inline double SSIMCalculation(
...
# 49 "/doner/libwebp/libwebp-31bea324/src/dsp/ssim.c" 3 4
   ((void) (0))
# 49 "/doner/libwebp/libwebp-31bea324/src/dsp/ssim.c"
                              ;
    return r;
  }
  return 1.;
...
static double SSIMGetClipped_C(const uint8_t* src1, int stride1,
  ...
  const int ymin = (yo - 3 < 0) ? 0 : yo - 3;
  const int ymax = (yo + 3 > H - 1) ? H - 1
                                                  : yo + 3;
  const int xmin = (xo - 3 < 0) ? 0 : xo - 3;
  const int xmax = (xo + 3 > W - 1) ? W - 1
                                                  : xo + 3;
  int x, y;
  src1 += ymin * stride1;
  src2 += ymin * stride2;
  for (y = ymin; y <= ymax; ++y, src1 += stride1, src2 += stride2) {
    for (x = xmin; x <= xmax; ++x) {
      const uint32_t w = kWeight[3 + x - xo]
                       * kWeight[3 + y - yo];
...
static double SSIMGet_C(const uint8_t* src1, int stride1,
  ...
  VP8DistoStats stats = { 0, 0, 0, 0, 0, 0 };
  int x, y;
  for (y = 0; y <= 2 * 3; ++y, src1 += stride1, src2 += stride2) {
    for (x = 0; x <= 2 * 3; ++x) {
...
# 121 "/doner/libwebp/libwebp-31bea324/src/dsp/ssim.c" 3 4
 ((void) (0))
# 121 "/doner/libwebp/libwebp-31bea324/src/dsp/ssim.c"
                     ;
  for (i = 0; i < len; ++i) {
...
static void VP8SSIMDspInit_body(void); void VP8SSIMDspInit(void) { do { static volatile VP8CPUInfo VP8SSIMDspInit_body_last_cpuinfo_used = (VP8CPUInfo)&VP8SSIMDspInit_body_last_cpuinfo_used; static pthread_mutex_t VP8SSIMDspInit_body_lock = 
# 143 "/doner/libwebp/libwebp-31bea324/src/dsp/ssim.c" 3 4
{ { 0, 0, 0, 0, PTHREAD_MUTEX_TIMED_NP, 0, 0, { 0, 0 } } }
# 143 "/doner/libwebp/libwebp-31bea324/src/dsp/ssim.c"
; if (pthread_mutex_lock(&VP8SSIMDspInit_body_lock)) break; if (VP8SSIMDspInit_body_last_cpuinfo_used != VP8GetCPUInfo) VP8SSIMDspInit_body(); VP8SSIMDspInit_body_last_cpuinfo_used = VP8GetCPUInfo; (void)pthread_mutex_unlock(&VP8SSIMDspInit_body_lock); } while (0); } static void VP8SSIMDspInit_body(void) {
  ...
  if (VP8GetCPUInfo != 
# 153 "/doner/libwebp/libwebp-31bea324/src/dsp/ssim.c" 3 4
                      ((void *)0)
# 153 "/doner/libwebp/libwebp-31bea324/src/dsp/ssim.c"
                          ) {
